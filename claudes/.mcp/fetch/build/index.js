...
export async function getRawTextString(request_url) {
    const response = await axios.get(request_url);
    const data = response.data;
    return data;
}
// Helper method to fetch rendered HTML content using a headless browser
async function getHtmlString(request_url) {
    let browser = null;
    let page = null;
    try {
...
export async function getMarkdownStringFromHtmlByTD(request_url, mainOnly = false) {
    ...
    turndownService.addRule('table', {
        ...
        replacement: function (content, node, _options) {
            ...
            const headerCells = Array.from(headerRow.querySelectorAll('th, td')).map((cell) => cell.textContent?.trim() || '');
            ...
            for (let i = 1; i < rows.length; i++) {
                const row = rows[i];
                const rowCells = Array.from(row.querySelectorAll('th, td')).map((cell) => cell.textContent?.trim() || '');
                markdown += `\n| ${rowCells.join(' | ')} |`;
            }
            return markdown + '\n';
        },
...
export async function getMarkdownStringFromHtmlByNHM(request_url, mainOnly = false) {
    const htmlString = await getHtmlString(request_url);
    ...
    const markdownString = NodeHtmlMarkdown.translate(htmlString, {}, customTranslators);
    return markdownString;
}
/**
