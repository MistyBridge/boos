...
const crashForensics = require('./crashForensics');
...
try {
  ...
  loadError = e;
  errReport.report('webTerminal', 'node-pty-load', e);
}
...
// ── serialized PTY write queue ─────────────────────────────────────────
// Windows ConPTY: concurrent native pty.write() calls (across many PTYs
// firing in the same tick — e.g. 8 boot injections at T+8s) can crash the
// process at the NATIVE layer: no JS exception fires, uncaughtException
// never runs, the process dies with exit code 1 and no stack. All writes
// are chained onto one promise tail so at most one native write is in
// flight at any instant. Order is preserved (FIFO).
//
// Sprint 42 hardening: 50ms settle gap BETWEEN requests (user decision
// 2026-08-07) — even a serialized queue can burst when 8 injections land
// in the same tick; the gap spaces native writes ~20/s max, well under
// ConPTY's stress threshold.
const WRITE_GAP_MS = parseInt(process.env.BOOS_PTY_WRITE_GAP_MS, 10) || 50;
let _writeTail = Promise.resolve();

// PTY Scanner data hooks — sessionId → Set<callback>
// Each callback receives output text (string, not Buffer).
const _dataHooks = new Map();
...
function spawn({ command, args = [], cwd, env, cols = 120, rows = 30, meta = {}, id, onData, onExit }) {
  ...
  if (process.platform === 'win32') {
    ...
    // Sprint 42 crash fix: useConptyDll=true was crashing the process at
    // the NATIVE layer on high-frequency spawns (Windows 11 + node-pty
    // 1.1.0): boot crash-reconnect spawns 8 PTYs, then stale-reclaim's
    // auto-resume spawns a 9th → process dies with exit code 1, no JS
    // stack, no uncaughtException. node-pty's bundled ConPTY DLL is
    // unstable under repeated spawn; the OS-native ConPTY (default when
    // useConpty=true, useConptyDll=false) is stable. Fixed by dropping
    // useConptyDll — Win10 1809+ ships the native ConPTY API.
  }
  const proc = pty.spawn(command, args, ptyOpts);
  crashForensics.note('pty-spawn', { sessionId: String(entryId).slice(-8), command, pid: proc.pid });
  // Handle async EPIPE errors from native stream layer — write to dead PTY.
  // Without this, unhandled 'error' events become uncaught exceptions.
  proc.on('error', (err) => {
    if (err.code === 'EPIPE' || err.message?.includes('EPIPE')) return; // ignore — PTY exited
    console.warn('[webTerminal] PTY error for', entryId, err.message);
  });
  const entry = {
    id: entryId,
    pty: proc,
  ...
  proc.onData((data) => {
    ...
    for (const ws of entry.sockets) {
      try {  ws.send(frame);  } catch (e) { errReport.report("webTerminal", "send", e); }
    }
    // Emit agent_status for workspace node colour (debounced 2s).
    _emitAgentStatus(entry.id, entry.sockets);
    // Trigger PTY Scanner hooks (Sprint 11)
    const hooks = _dataHooks.get(entry.id);
    if (hooks) {
      const text = typeof data === 'string' ? data : data.toString();
      for (const cb of hooks) cb(text);
    }
  ...
  proc.onExit(({ exitCode, signal }) => {
    ...
    for (const ws of entry.sockets) {
      try { ws.send(frame); } catch (e) { errReport.report('webTerminal', 'ws-send-exit', e, { entryId }); }
    }
    if (entry.onExitExtra) { try { entry.onExitExtra({ exitCode, signal }); } catch (e) { errReport.report('webTerminal', 'onExitExtra', e, { entryId }); } }
    // Sprint 20: notify identity card that this session's PTY has exited.
    try { require('./agentBus/store').onSessionExited(entryId); } catch (e) { errReport.report('webTerminal', 'onSessionExited', e, { entryId }); }
    // 30s deferred cleanup — unref so it never holds the process open in
    // tests/scripts where a PTY exits and the harness waits on the event loop.
    setTimeout(() => {
  ...
  if (prev && !prev.exitedAt) {
    for (const ws of prev.sockets) {
      try {  ws.close(1012, 'terminal respawned');  } catch (e) { errReport.report("webTerminal", "close", e); }
    }
    prev.sockets.clear();
    try {  prev.pty.kill();  } catch (e) { errReport.report("webTerminal", "kill", e); }
  }
  return entry;
}
...
function attach(id, ws) {
  ...
  if (!entry) {
    try {  ws.close(4404, 'no such terminal');  } catch (e) { errReport.report("webTerminal", "close", e); }
    return;
  }
  ...
  for (const other of entry.sockets) {
    try {  other.close(4001, 'displaced by another client');  } catch (e) { errReport.report("webTerminal", "close", e); }
  }
  entry.sockets.clear();
    ...
    switch (event.type) {
      case 'input':
        if (typeof event.data === 'string') {
          ...
          // Sprint 42: route ALL writes through the serialized write() —
          // browser input must never race native pty.write() with system
          // injections (concurrent ConPTY writes crash the process).
          try { write(id, event.data); } catch (e) {
            // EPIPE: PTY process exited between the check and the write — ignore.
          }
        }
        break;
      case 'resize':
        if (Number(event.cols) > 0 && Number(event.rows) > 0) {
          try {  entry.pty.resize(Number(event.cols), Number(event.rows));  } catch (e) { errReport.report("webTerminal", "resize", e); }
        }
        break;
      case 'kill':
        kill(id);
...
function kill(id) {
  ...
  try {  entry.pty.kill();  } catch (e) { errReport.report("webTerminal", "kill", e); }
  return true;
}
 ...
 *
 * Sprint 42 fix: serialized — every write is chained onto the global
 * _writeTail so native pty.write() calls never overlap. Concurrent native
 * writes on Windows (ConPTY) crash the whole process with no JS stack
 * (observed: 8 parallel boot injections at T+8s → exit code 1, no
 * UNCAUGHT EXCEPTION). The queue is fire-and-forget; callers don't await.
...
function write(sessionId, data) {
  ...
  if (!entry || !entry.pty || entry.exitedAt) {
    // Sprint 42: no silent no-op — record why the write was dropped.
    errReport.skip('webTerminal', 'write', 'no live pty', {
      sessionId, hasEntry: !!entry, hasPty: !!(entry && entry.pty),
      exitedAt: entry && entry.exitedAt,
    });
    return;
  }
  _writeTail = _writeTail
    .then(() => {
      crashForensics.note('pty-write', { sessionId: String(sessionId).slice(-8), bytes: String(data).length });
      // PTY may have exited while queued — then drop silently.
      const e = sessions.get(sessionId);
      if (!e || !e.pty || e.exitedAt) {
        errReport.skip('webTerminal', 'write', 'pty died while queued', { sessionId });
        return;
      }
      // Sprint 42 crash fix: node-pty's onExit event can lag or never fire
      // when the child dies hard (ConPTY kill). Writing to a dead PTY
      // handle crashes the process at the native layer (no JS exception).
      // Probe the OS pid directly — process.kill(pid, 0) is a no-op probe
      // that throws ESRCH when the process is gone.
      try {
        process.kill(e.pty.pid, 0);
      } catch (probeErr) {
        errReport.skip('webTerminal', 'write', 'pid gone (probe)', { sessionId, pid: e.pty.pid });
        return;
      }
      if (process.env.BOOS_PTY_DEBUG) {
        console.log(`[pty.write→${String(sessionId).slice(-8)}] len=${data.length} "${String(data).slice(0, 60).replace(/\x03/g, '<CTRL-C>').replace(/\x1b/g, '<ESC>').replace(/\r/g, '\\r').replace(/\n/g, '\\n')}"`);
      }
      e.pty.write(data);
    })
    .catch((err) => {
      // EPIPE: PTY process exited between the check and the write.
      errReport.report('webTerminal', 'write', err, { sessionId });
    })
    // 50ms gap between requests — paces native writes even when a burst
    // of injections lands in the same tick (see WRITE_GAP_MS above).
    .then(() => new Promise((r) => setTimeout(r, WRITE_GAP_MS)));
}
...
function killAll() {
  for (const e of sessions.values()) {
    try {  e.pty.kill();  } catch (e) { errReport.report("webTerminal", "kill", e); }
  }
}
...
function gracefulKillAll(gracefulTimeoutMs = 5000) {
  return new Promise((resolve) => {
    ...
    for (const e of entries) {
      ...
      // NOTE: this path bypasses the serialized write() queue — a burst of
      // \x03 across N PTYs in the same tick is a ConPTY-native crash risk
      // (Sprint 42). Marker + debug so a crash here is diagnosable.
      crashForensics.note('pty-ctrl-c', { sessionId: String(e.id).slice(-8) });
      try {  e.pty.write('\x03');  } catch (e) { errReport.report("webTerminal", "write", e); }
...
/**
 * Register a PTY data callback. Fires on every output chunk after WS send.
 * Returns an unsubscribe function. Callback receives a string (not Buffer).
 * Safe: no-op if session doesn't exist or has exited.
 * @param {string} sessionId
 * @param {(text: string) => void} callback
 * @returns {() => void} unsubscribe
 */
function onData(sessionId, callback) {
  if (!_dataHooks.has(sessionId)) _dataHooks.set(sessionId, new Set());
  _dataHooks.get(sessionId).add(callback);
  return () => {
    const hooks = _dataHooks.get(sessionId);
...
module.exports = {
  ...
  get,
  write,
  flushWrites: () => _writeTail.catch(() => {}), // tests/scripts: await queued writes
  count,
  onData,
  killAll,
  gracefulKillAll,
