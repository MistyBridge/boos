...
function cancelDevtunnelLogin() {
  ...
  try {  login.child.kill();  } catch (e) { errReport.report("tunnelDevtunnel", "kill", e); }
  return loginSnapshot();
}
...
async function resetDevtunnelTunnelId({ findBinary }) {
  let prevId = null;
  try {  const cfg = await loadConfig(); prevId = cfg?.devtunnel?.tunnelId || null;  } catch (e) { errReport.report("tunnelDevtunnel", "loadConfig", e); }
