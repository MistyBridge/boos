...
async function update(id, patch) {
  ...
  return withFileLock(FILE, async () => {
    ...
    if (typeof patch.autoSupervisorEnabled === 'boolean') allowed.autoSupervisorEnabled = patch.autoSupervisorEnabled;
...
// Sprint 13.2: extended to { sandbox: "PM"|"PMO"|"SE", write: boolean }.
// Sprint 32: added PMO level — PMO bypasses sandbox like PM but cannot
//           create/modify DAGs (PM-only privilege).
// Backward-compat: legacy "PM"/"SE" string values auto-normalized.

const VALID_SANDBOX_LEVELS = new Set(['PM', 'PMO', 'SE']);

function normalizeLevel(raw) {
  if (!raw || typeof raw === 'string') {
    const sandbox = VALID_SANDBOX_LEVELS.has(raw) ? raw : 'SE';
    return { sandbox, write: sandbox === 'PM' || sandbox === 'PMO' };
  }
  if (typeof raw === 'object') {
    const sandbox = VALID_SANDBOX_LEVELS.has(raw.sandbox) ? raw.sandbox : 'SE';
    return {
      sandbox,
      write: sandbox === 'PM' || sandbox === 'PMO' ? true : Boolean(raw.write),
    };
  }
