...
function createDecision({ agent_uid, agent_name, workspace, title, content, urgent }) {
  _ensureDirs();
  const decId = _genId();
...
function approveDecision(decisionId, approver) {
  ...
  return { ok: true, decision_id: decisionId, status: 'approved', approver };
}
