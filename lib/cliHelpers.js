...
function resolveCommand(commandRaw, userArgs = [], shell = 'direct') {
  ...
  // Non-absolute command with shell === 'direct'.
  // Windows: bare names historically ran through cmd.exe. Linux/macOS:
  // let the OS resolve the bare command via PATH — cmd.exe doesn't exist.
...
function probeCli(command) {
  ...
  } catch (e) { errReport.report("cliHelpers", "trim", e); }
