...
function createIdleWatcher({
  ...
  function check() {
    ...
    // Criterion 4: managed agents with pending inbox work?
    // Even if PTY is dead, agents might have pending tasks — auto-resume
    // handles recovery on frontend reconnect.  Without this check the
    // server could idle-shutdown while agents still have work queued.
    let hasPendingAgentWork = false;
    try {
      ...
      if (fs.existsSync(inboxDir)) {
        ...
        for (const f of files) {
          try {
            ...
            if ((data.pending || []).length + (data.in_progress || []).length > 0) {
              hasPendingAgentWork = true;
              break;
            }
          } catch (e) { errReport.report("idleWatcher", "parse", e); }
        }
      }
    } catch (e) { errReport.report("idleWatcher", "parse", e); }

    const isActive = hasRunningSession || hasRecentHeartbeat || hasMcpConnections || hasPendingAgentWork;
