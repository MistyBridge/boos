...
async function detectClaude(descSet, cwd) {
  ...
  for (const name of names) {
    ...
    } catch (e) { errReport.report("sessionBindingDetect", "push", e); }
  }
...
async function detectCopilot(descSet) {
  ...
  for (const name of names) {
    ...
    if (!descSet.has(pid)) continue;
    let mtime = 0;
    try {  mtime = (await fsp.stat(path.join(dir, name))).mtimeMs;  } catch (e) { errReport.report("sessionBindingDetect", "stat", e); }
...
function codexSessionRoots() {
  ...
  for (const base of bases) {
    for (const child of listChildDirsSync(base)) {
      ...
      try {  if (fs.existsSync(path.join(cand, 'sessions'))) homes.add(cand);  } catch (e) { errReport.report("sessionBindingDetect", "existsSync", e); }
    }
  }
