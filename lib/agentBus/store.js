 ...
 */
// Sprint 22: simplified — only identity card lookup. Heuristic passes
// (title, workspace, cwd basename) removed; sessions now have agentUid.
async function resolveBoosSessionForAgent(name, workspace) {
  try {
    ...
    // Pass 1 (only): identity_by_name_ws → boos_session_id.
    const db = _syncLoad();
    const nwKey = name + '|' + workspace;
    const uid = db.identity_by_name_ws[nwKey];
    if (uid) {
