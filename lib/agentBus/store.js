...
const EMPTY_DB = { agents: {}, tasks: {}, name_ws_index: {}, sessions: {}, identities: {}, identity_by_boos_session: {}, identity_by_mcp_session: {}, identity_by_name_ws: {}, dags: {}, dag_tasks: {} };
...
function _syncLoad() {
  try {
    ...
    // Sprint 31: DAG task system schema extension.
    db.dags = db.dags || {};
...
// Migrate an agent from a legacy random UID to a deterministic UID.
// Updates ALL references: agents table, name_ws_index, tasks, sessions.
// Atomic within withFileLock — either all references are updated or none.
async function migrateAgentUid(oldUid, newUid) {
  ...
  return withFileLock(FILE, async () => {
    ...
    // 2. Update name_ws_index.
    const key = `${agent.name}|${agent.workspace}`;
    db.name_ws_index[key] = newUid;

    // 3. Update all task references.
    ...
    // 4. Update all session references.
    for (const [sid, s] of Object.entries(db.sessions || {})) {
...
async function insertTask(task) {
  return withFileLock(FILE, async () => {
    ...
    db.tasks[task.task_id] = {
      ...
      message_type: task.message_type || 'request',  // Sprint 19: request | response
...
/** @deprecated — use writeIdentity(agentUid, { mcp_session_id }) */
async function bindMcpSession(mcpSessionId, agentUid) {
  return writeIdentity(agentUid, { mcp_session_id: mcpSessionId });
}

/** @deprecated — use writeIdentity(agentUid, { mcp_session_id: null }) */
...
/**
 * Sprint 30: Resolve agent UID → BOOS session ID via identity card.
 *
 * Lookup order:
 *   1. identity card (boos_session_id field)
 *   2. sessions.json scan (agentUid field)
 *
 * Returns the BOOS session ID string (sess-…) or null.
 * This is the ONLY function that maps Agent UID → BOOS session ID;
 * all code paths that need a BOOS session from a UID go through here.
 */
async function resolveBoosSessionForAgent(agentUid) {
  ...
  try {
    ...
    // Pass 1: identity card → boos_session_id (authoritative).
    const db = _syncLoad();
    ...
    // Pass 2: sessions.json scan by agentUid field (fallback for
    // sessions whose identity card hasn't been written yet).
...
// ── Sprint 22: Unified atomic identity write ───────────────────────────
// Replaces upsertIdentity, linkIdentityToSession, bindMcpSession,
// unbindMcpSession, and onSessionExited.  One function, all three reverse
// indices (boos_session, mcp_session, name|workspace) updated atomically
// inside a single withFileLock.  No more index drift.

/**
 * Atomically write identity fields AND all three reverse indices.
 *
 * @param {string} agentUid
 * @param {object} fields  — { name, workspace, role, boos_session_id,
 *                            mcp_session_id, cwd, pty_pid, ... }
 * @returns {object} the merged identity card
 */
async function writeIdentity(agentUid, fields) {
  return withFileLock(FILE, async () => {
    ...
    // ── 2. mcp_session_id ↔ uid  (was never maintained consistently!) ──
...
/**
 * Rebuild all three reverse indices from identity cards.
 * Idempotent — safe to call on every startup.
 * No heuristics — only uses explicit fields from existing identity cards.
 */
...
// ── @deprecated (Sprint 22) — use writeIdentity() instead ─────────────

/** @deprecated — use writeIdentity(agentUid, fields) */
...
// ── @deprecated Identity card lifecycle (Sprint 20 → 22) ──────────────
// writeIdentity() replaces all five functions below.  These wrappers
// exist for backward compat only; new code MUST use writeIdentity().

/** @deprecated — use writeIdentity(agentUid, { boos_session_id, cwd, pty_pid }) */
async function linkIdentityToSession(agentUid, sessionId, cwd, ptyPid) {
  ...
  return writeIdentity(agentUid, {
    boos_session_id: sessionId,
    cwd: cwd || undefined,
    pty_pid: typeof ptyPid === 'number' ? ptyPid : undefined,
  });
}

/** @deprecated — use writeIdentity(agentUid, { pty_pid: 0 }) */
...
/**
 * Auto-register an identity from MCP context when ctx.uid is null.
 * Called by handlers dispatch() before rejecting "not registered".
 * Returns { uid, identity } if identity can be derived, or null.
 */
function autoResolveIdentity(mcpSessionId) {
  if (!mcpSessionId) return null;
  const db = _syncLoad(); // @stale-ok: read-only
  // Try MCP session binding first.
  let uid = db.identity_by_mcp_session[mcpSessionId];
  if (!uid) {
    // Fallback: transport session store.
...
async function bootstrapIdentities() {
  return withFileLock(FILE, async () => {
    ...
    // Sprint 22: rebuildAllIndices() replaces heuristic session cross-reference.
    // Session→agentUid linking is now done at launch time; bootstrap only needs
    // to rebuild index structures from explicit identity card fields.
    // Note: rebuildAllIndices() runs inside its own withFileLock — safe to call
    // from inside this lock since we already hold it, but it will re-acquire.
    // Since the indices have been cleared and rebuilt above, we just need to
    // save what we've built so far and let rebuildAllIndices handle the indices.
    // Actually: we already rebuilt basic identity cards with name_ws index above.
    // rebuildAllIndices is called separately at startup in server.js, so we can
    // skip it here to avoid double-work and potential deadlock.
...
module.exports = {
  ...
  // Identity cards — writeIdentity is the canonical write path (Sprint 22)
  writeIdentity, rebuildAllIndices,
  // @deprecated wrappers (still work, delegate to writeIdentity)
  upsertIdentity, linkIdentityToSession, onSessionExited,
  bindMcpSession, unbindMcpSession,
  // Read-only identity lookup
  getIdentity, getIdentityByBoosSession, getAgentUidByMcpSession, autoResolveIdentity,
  bootstrapIdentities, validateIdentity, resolveBoosSessionForAgent,
