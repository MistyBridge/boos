...
async function registerAgent({ name, intro, workspace, role, capabilities, project,
                                cliSessionId, _internalRegister, _systemUid }) {
  if (!name || !workspace) {
  ...
  // Sprint 42: system agents (HR) have no CLI session — they register
  // with _internalRegister + _systemUid instead.
  if (!cliSessionId && !_systemUid && !_internalRegister) {
  ...
  // Sprint 42 fix: in-process system agents (HR, generalist — _internalRegister)
  // bypass the gate — they run at boot BEFORE the workspace necessarily
  // exists (fresh DB / post-purge), and they are BOOS-internal, not external
  // workers. External MCP register_agent calls never set _internalRegister.
  const wsExists = store.listAllAgents().some((a) => a.workspace === workspace);
  const canCreate = role === 'supervisor' || role === 'root' || !!_internalRegister;
  ...
  // uid = cliSessionId, always.  System agents get their own stable uid
  // (e.g. 'agent_hr') — never collides with real Claude sessions.
  const uid = cliSessionId || _systemUid || ROOT_UID;
  ...
  if (existingByName && existingByName.uid !== uid) {
    // Sprint 42: system agents (HR) reuse the existing card instead of
    // migrating it — a real Claude session (transport auto-registered)
    // already owns that uid; overwriting it to 'agent_hr' would orphan it.
    if (_systemUid) {
      await store.touchAgent(existingByName.uid);
      if (cleanProject && !existingByName.project) {
        await store.setAgentProject(existingByName.uid, cleanProject);
      }
      return {
        ok: true, uid: existingByName.uid, reconnected: true,
        pending_tasks: await store.countPendingTasks(existingByName.uid),
        registered_at: existingByName.registered_at,
        reused_system_uid: true,
      };
    }
...
// ── Sprint 42 robustness: snapshot + workspace-scoped purge ─────────────
...
// Rotating snapshot of the registry file before destructive operations.
// Keeps the last SNAPSHOT_KEEP backups so accidental purges are rollback-able.
function snapshot() {
  try {
    const { DB_PATH } = require('./storeCore');
    const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const dir = path.dirname(DB_PATH);
    const bak = path.join(dir, `agent-bus.snap-${ts}.json`);
    fs.copyFileSync(DB_PATH, bak);
    // rotate: keep newest SNAPSHOT_KEEP
    const snaps = fs.readdirSync(dir)
      .filter((f) => f.startsWith('agent-bus.snap-'))
      .sort();
    while (snaps.length > SNAPSHOT_KEEP) {
      fs.unlinkSync(path.join(dir, snaps.shift()));
    }
    return { ok: true, snapshot: bak };
  } catch (e) {
    errReport.report('registry', 'snapshot', e);
    return { ok: false, error: e.message };
  }
}

// Remove ALL agents registered under a workspace (identity cards + indexes).
// Destructive — callers must be supervisor/PM and should snapshot() first.
async function purgeWorkspace(workspace) {
  const wsCheck = validateWorkspace(workspace);
  if (!wsCheck.valid) return { ok: false, error: wsCheck.reason };
  const all = store.listAllAgents();
  const targets = all.filter((a) => a.workspace === workspace && a.uid !== ROOT_UID);
  if (targets.length === 0) return { ok: true, removed: 0, workspace };

  const removed = [];
  await store.withFileLock('purge-ws-' + workspace, async () => {
    for (const a of targets) {
      store.deleteAgent(a.uid);
      removed.push(a.uid);
    }
  });
  return { ok: true, removed: removed.length, workspace, uids: removed };
}
