...
function forceDeregisterAgent(uid) {
  ...
  // Sprint 39: clean up stale tasks in agent-bus.json for this agent.
  // Since Sprint 35, tasks live in per-agent inbox files, but agent-bus.json
  // may still hold orphaned tasks from before the refactor or from notification
  // loops. Without cleanup, these cause phantom activeTasks counts.
  try {
    const storeTasks = require('./storeTasks');
    const allTasks = storeTasks.listMyTasks(uid);
    for (const t of allTasks) {
      storeTasks.deleteTask(t.task_id);
    }
  } catch (e) {
