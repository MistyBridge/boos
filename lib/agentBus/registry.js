...
async function registerAgent({ name, intro, workspace, role, capabilities, project, _internalRegister }) {
  ...
  if (existing) {
    ...
    // Sprint 28: UID migration — if the existing agent has a legacy
    // random UID (from pre-deterministic era), migrate all references
    // to the new deterministic UID. One-time, idempotent.
    const deterministicUid = _generateUid(cleanName, workspace);
    if (existing.uid !== deterministicUid) {
      const migration = await store.migrateAgentUid(existing.uid, deterministicUid);
      if (migration.ok && migration.migrated) {
        const pendingCount = store.countPendingTasks(deterministicUid);
        return {
          ok: true,
      ...
      // Migration failed (e.g. target UID collision) — fall through,
      // keep old UID. Non-fatal.
    }
  ...
  const uid = _generateUid(cleanName, workspace);
...
// Deterministic UID from name + workspace. Same role in same workspace
// always gets the same UID, even across reconnections from different
// Claude sessions. This keeps CLAUDE.md routing tables stable forever.
function _generateUid(name, workspace) {
