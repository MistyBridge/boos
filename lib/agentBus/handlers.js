...
async function dispatch(toolName, args, ctx) {
  ...
  switch (toolName) {
    ...
    // ── Session / infra ──────────────────────────────────────────────
    case 'launch_agent_session': return handlersSession._launchAgentSession(args, ctx);
    ...
    case 'pmo_poll':           return handlersDag._pmoPoll(args, ctx);
...
async function _register(args, ctx) {
  ...
  try {
    ...
    await store.writeIdentity(result.uid, {
      name: agent.name, workspace: agent.workspace, role: agent.role || 'worker',
      mcp_session_id: ctx.sessionId, boos_session_id: boosSessionId || undefined,
    });
  ...
  return {
    ...
    reconnected: result.reconnected || false, pending_tasks: result.pending_tasks || 0,
...
// Re-export _internalLaunchAgentSession for backward compat with notifications.js.
const { _internalLaunchAgentSession } = handlersSession;
