...
async function dispatch(toolName, args, ctx) {
  // Sprint 20: auto-registration.  If the caller is not registered but
  // their MCP session is known, auto-derive identity from the identity card
  // instead of rejecting with "not registered".  This handles agents that
  // reconnect after a BOOS restart — their MCP session ID is still linked
  // to their persistent identity, so they pick up right where they left off.
  ...
  switch (toolName) {
    ...
    // check_decisions removed (event-driven via SSE push notifications)
    // check_root_response removed (event-driven — root replies delivered via SSE + auto-wake)
    ...
    case 'list_agent_cards':  return _listAgentCards(args, ctx);
    ...
    case 'dag_create':         return _dagCreate(args, ctx);
    case 'dag_add_task':       return _dagAddTask(args, ctx);
    case 'dag_activate':       return _dagActivate(args, ctx);
    case 'dag_status':         return _dagStatus(args, ctx);
    case 'dag_cancel':         return _dagCancel(args, ctx);
    case 'dag_submit_task':    return _dagSubmitTask(args, ctx);
    case 'dag_approve_task':   return _dagApproveTask(args, ctx);
    case 'dag_reject_task':    return _dagRejectTask(args, ctx);
    case 'dag_my_tasks':       return _dagMyTasks(args, ctx);
    case 'dag_reassign_task':  return _dagReassignTask(args, ctx);
    case 'dag_list':           return _dagList(args, ctx);
    case 'dag_sleep_agent':    return _dagSleepAgent(args, ctx);
    case 'dag_wake_agent':     return _dagWakeAgent(args, ctx);
    case 'pmo_poll':           return _pmoPoll(args, ctx);
...
async function _register(args, ctx) {
  ...
  // Sprint 30: resolve BOOS session by UID (previously name+workspace).
  // writeIdentity atomically updates all 3 reverse indices.
  try {
    const agent = store.getAgent(result.uid);
    const boosSessionId = await store.resolveBoosSessionForAgent(result.uid);
    ...
    await store.writeIdentity(result.uid, {
      ...
      boos_session_id: boosSessionId || undefined,
    });
  } catch (e) {
...
async function _sendTask(args, ctx) {
  ...
  // ── Sprint 19: message_type validation ───────────────────────────────
  ...
  // Response chain validation: response must reply to an existing task
  // and must terminate at the original requester.
  if (msgType === 'response') {
    if (!args.reply_to) {
      return { error: 'message_type "response" requires reply_to (the task being replied to)' };
    }
    const parent = store.getTask(args.reply_to);
    if (!parent) {
      return { error: 'reply_to task not found: ' + args.reply_to };
    }
    // Follow the chain to find the root request.
    const root = _findRootTask(args.reply_to);
    // Response must go to someone in the root request pair (origin sender or origin receiver).
    const rootSender = root.sender_uid;
    const rootReceiver = root.receiver_uid;
    if (!args.to_uid) {
      // Auto-target: reply to whoever sent you the parent task.
      args.to_uid = parent.sender_uid;
    }
    if (args.to_uid !== rootSender && args.to_uid !== rootReceiver) {
      return { error: 'response chain must terminate at the original requester. Valid targets: ' + rootSender + ' (root sender) or ' + rootReceiver + ' (root receiver)' };
    }
  }
  ...
  if (r.ok) {
    // Sprint 19: Auto-wake receiver — system-level enforcement of event-driven model.
    // Every send_task automatically triggers wake_agent so the receiver never
    // misses a task due to being asleep. Best-effort, non-fatal on failure.
    const receiverUid = r.task?.receiver_uid;
    if (receiverUid) {
      try {
        await notifications.wakeAgent(receiverUid, {
          urgency: args.priority === 'high' ? 'urgent' : 'normal',
          sender_name: myself.name,
          sender_uid: ctx.uid,
          task_count: 1,
        });
      } catch {} // Non-fatal: task is already queued; manual wake_agent still works.
    }
...
// ── Sprint 19: Request-Response chain helpers ─────────────────────────────

// Walk the reply_to chain to find the root request (the first task with no parent).
function _findRootTask(taskId, maxDepth = 20) {
  ...
  return current; // root task (message_type === 'request')
}
...
async function _respondTask(args, ctx) {
  ...
  // ── Sprint 20 fix: respond_task IS the response delivery mechanism ──
  // respond_task completes the task AND notifies the sender (queue.js emits
  // outboxEvents('task_completed') → notifications._onTaskCompleted pushes the
  // result to the sender + wakes them). So calling respond_task already
  // satisfies "no silent completion" — the sender always learns the outcome.
  //
  // The old Sprint 19 gate REQUIRED a separate send_task(message_type=
  // "response") BEFORE respond_task would succeed. That double-bookkeeping was
  // invisible to the model and deadlocked every normal reply: agents called
  // respond_task, got rejected, and the task stuck in_progress forever. This
  // was the "respond_task 无法提交" bug. The result string carried by
  // respond_task is itself the response payload — no separate message needed.
...
async function _broadcast(args, ctx) {
  ...
  // Sprint 19: Auto-wake all broadcast recipients.
  // Broadcast messages are urgent by nature — wake every agent so they see it.
  if (r.ok && uids.length > 0) {
    const myself = store.getAgent(ctx.uid);
    Promise.all(uids.map((uid) =>
      notifications.wakeAgent(uid, {
        urgency: 'normal',
        sender_name: myself?.name || '系统',
        sender_uid: ctx.uid,
      }).catch(() => {})
    )).catch(() => {});
  }
...
async function _listAgentCards(args, ctx) {
  ...
  // Gather active PTY session IDs for the workspace.
  const ptyByUid = {};
  try {
  ...
  // Optionally enrich with live PTY status from webTerminal.
  ...
  const cards = agents.map((a) => {
    ...
    return {
      ...
      // ── Status ──
      status: state.state,               // 'idle' | 'busy' | 'unresponsive'
      active_tasks: state.taskCount,      // pending + in_progress
      has_pty: hasLivePty,               // BOOS terminal session alive
...
async function _wakeAgent(args, ctx) {
  ...
  // ID Card: only supervisors (PMs) can wake agents — same gate as wake_all
  const supErr = _requireSupervisor(ctx);
  if (supErr) return supErr;
...
async function _launchAgentSession(args, ctx) {
  ...
  // Sprint 30: prefer agent_uid as primary index; fall back to agent_name
  // + workspace for backward compat.
  let agent;
  if (args.agent_uid) {
    agent = store.getAgent(args.agent_uid);
  } else if (args.agent_name) {
    // @deprecated — use agent_uid
  ...
  // Ensure returned workspace matches agent's registered workspace.
  ...
  // Sprint 30: resolve BOOS session from agent UID via identity card.
  // Falls back to sessions.json agentUid field if identity card absent.
  let existing = null;
  try {
...
// ── Sprint 31: DAG Task System Handlers ──────────────────────────────────
...
async function _dagActivate(args, ctx) {
  _requirePMorPMO(ctx);
  const dagStore = require('./dagStore');
  const result = await dagStore.activateDag(args.dag_id);
...
async function _dagApproveTask(args, ctx) {
  const taskSystem = require('./taskSystem');
  return taskSystem.approveTask(args.task_id, ctx.uid, {
...
async function _dagRejectTask(args, ctx) {
  const taskSystem = require('./taskSystem');
  return taskSystem.rejectTask(args.task_id, ctx.uid, {
    comment: args.comment,
  });
}
...
async function _dagSleepAgent(args, ctx) {
  ...
  // Auth: PM (supervisor) can sleep workers; PMO can sleep PM (supervisor).
  ...
  // Delegate to sleepManager (Phase 6).
  try {
...
async function _dagWakeAgent(args, ctx) {
  ...
  try {
    ...
    // Fallback: use existing wake_agent handler.
    return _wakeAgent({ target_uid: args.target_uid }, ctx);
  }
}
