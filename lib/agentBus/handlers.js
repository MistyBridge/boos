...
async function _register(args, ctx) {
  ...
  // Sprint 22: single atomic writeIdentity — all fields + all 3 reverse indices
  // in one withFileLock.  No more __pending__ sentinels, no index drift.
  try {
    const agent = store.getAgent(result.uid);
    const boosSessionId = await store.resolveBoosSessionForAgent(agent.name, agent.workspace);
