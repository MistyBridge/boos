...
async function dispatch(toolName, args, ctx) {
  ...
  // Sprint 42: also fills workspace when the transport bound uid but not
  // the workspace (header identity path).
  if ((!ctx.uid || !ctx.workspace) && toolName !== 'register_agent') {
    const auto = store.autoResolveIdentity(ctx.sessionId);
    if (auto) {
      ctx.uid = auto.uid;
      ctx.workspace = auto.identity?.workspace || ctx.workspace || null;
