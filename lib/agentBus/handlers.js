...
async function _register(args, ctx) {
  ...
  // Sprint 13: write identity card — cross-reference agent UID ↔ MCP session ID.
  try {
    const agent = store.getAgent(result.uid);
    await store.upsertIdentity(result.uid, {
      name: agent.name,
...
async function _respondTask(args, ctx) {
  if (!ctx.uid) return { error: 'not registered — register_agent first' };
  const r = await queue.respondTask(args.task_id, ctx.uid, args.result);
  if (r.ok) return { ok: true };
  return { error: r.error };
}
...
async function _launchAgentSession(args, ctx) {
  ...
  // Try agent-bus sessions table first (by UID), then name-based matching.
  let existing = null;
  ...
  try {
    ...
    const merged = {
      ...
      mcpServers: {
        ...
        'agent-bus': {
          type: 'sse',
          url: 'http://127.0.0.1:' + (process.env.BOOS_PORT || '7777') + '/mcp/sse',
        },
        filesystem: fsConfig,
      },
    };
...
async function _internalLaunchAgentSession(agentUid, agentName, agentWorkspace) {
  try {
    ...
    // Check identity card first — deterministic.
