...
const DAG_TOOLS = [
  ...
  {
    name: 'dag_wake_agent',
    description: 'Immediately wake a sleeping agent by injecting check_inbox into their PTY.',
    inputSchema: {
      type: 'object',
      properties: { target_uid: { type: 'string', description: 'Agent UID to wake.' } },
      required: ['target_uid'],
    },
  },
  {
    name: 'pmo_poll',
    description: 'PMO-only: full team health check. If all workers idle, ask PM for progress. If PM unresponsive (2 consecutive failures), execute PM recovery.',
    inputSchema: {
      type: 'object',
      properties: { workspace: { type: 'string', description: 'Workspace to poll. Default: current.' } },
      required: [],
    },
  },
  {
