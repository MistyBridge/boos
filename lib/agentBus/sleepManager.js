...
//
// Sprint 38 v2: _injectCommand uses plain \r injection by default.
// Paste-bracket is opt-in (BOOS_PTY_PASTE_MODE=1) — Ink TUI does NOT
// process bracketed-paste, so it's off by default.
...
async function sleep(targetUid, { wakeAfterMinutes = 5 } = {}) {
  ...
  // Inject /compact into PTY (single-shot: command + \n).
  let compacted = false;
  let ptyError = null;
  const session = await _findSessionByUid(targetUid);
  if (session) {
    try {
      _injectCommand(session.id, COMPACT_CMD);
      compacted = true;
    } catch (e) {
      ptyError = e.message;
    }
...
async function wake(targetUid) {
  ...
  // Inject check_inbox via PTY (single-shot: command + \n).
  let ptyInjected = false;
  let ptyError = null;
  const session = await _findSessionByUid(targetUid);
  if (session) {
    try {
      _injectCommand(session.id, WAKE_CMD);
      ptyInjected = true;
    } catch (e) {
      ptyError = e.message;
    }
