...
async function start(workspace) {
  ...
  taskTimeout.start(store, async (task) => {
    try {
      ...
      if (senderAg) {
        const sm = _findSession(allSessions, senderAg.name, senderAg.workspace);
        if (sm) _writeToPty(sm.id, '\n[agent-bus] ⏰ 任务超时 #' + task.task_id + ': 已自动取消（30分钟无响应）\n');
      }
      // Notify receiver.
      const recvAg = store.getAgent(task.receiver_uid);
      if (recvAg) {
        const rm = _findSession(allSessions, recvAg.name, recvAg.workspace);
...
async function _handleHrTask() {
  ...
  for (const t of tasks) {
    ...
    try {
      ...
      if (result.ok) {
        queue.respondTask(t.task_id, _hrAgentUid,
      ...
      queue.respondTask(t.task_id, _hrAgentUid, `❌ 招募异常: ${e.message}`);
    }
  }
}
