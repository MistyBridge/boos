...
const errReport = require('../errorReport');   // Sprint 42: module-level (was function-local — ReferenceError fix)
...
async function start(workspace) {
  ...
  // Sprint 38 fix: wire dagEngine SSE notifications.
  // Without this, dagEngine._notify is always null and DAG progress events
  // (task_approved, dag_activated, dag_completed, task_escalated) are lost.
  try {
    const dagEngine = require('./dagEngine');
    dagEngine.setNotify((event, data) => {
      try {
        const { notifyAgent } = require('./transport');
        // Broadcast DAG events to PM + involved agents via SSE.
        const uids = new Set();
        if (data.executor_uid) uids.add(data.executor_uid);
        if (data.reviewer_uid) uids.add(data.reviewer_uid);
        // Also notify the PM (supervisor).
        const store = require('./store');
        for (const [, agent] of store.getRegisteredAgents ? store.getRegisteredAgents() : []) {
          if (agent.role === 'supervisor') uids.add(agent.uid);
        }
        for (const uid of uids) {
          try { notifyAgent(uid, `notifications/agent_bus/${event}`, data); } catch {}
        }
      } catch (e) { errReport.report("notifications", "add", e); }
    });
  } catch (e) { errReport.report("notifications", "add", e); }
  ...
  const results = await Promise.allSettled([
    ...
    // Sprint 42 fix: HR is an in-process system agent with NO Claude CLI
    // session of its own, so registerAgent's cli_session_id requirement
    // always failed (logged "HR Agent registered: undefined"). Register
    // as a system agent: fixed uid, reuse existing card if present.
    registry.registerAgent({
      name: 'HR Agent',
      intro: 'BOOS 内嵌 HR Agent — 自动角色招募系统。',
      workspace: ws, role: 'worker', capabilities: ['recruitment', 'hr'],
      _internalRegister: true,
      _systemUid: 'agent_hr',
    }).then((hrResult) => {
  ...
  taskAnalytics.analyticsEvents.on('recruitment_suggested', async (suggestion) => {
    ...
    } catch (e) { errReport.report("notifications", "enqueue", e); }
  });
  ...
  try {
    ...
    if (decisionSystem.decisionsEvents) {
      decisionSystem.decisionsEvents.on('changed', ({ event }) => {
        ...
        } catch (e) { errReport.report("notifications", "_frontendNotify", e); }
      });
    }
  ...
  _staleTimer = setInterval(async () => {
    try {
      ...
      for (const f of files) {
        ...
        } catch (e) { errReport.report("notifications", "releaseAndDrain", e); }
      }
      if (n > 0) console.log('[boos] stale-reclaim:', n, 'tasks in_progress→pending');
    } catch (e) { errReport.report("notifications", "log", e); }
  }, 60_000);
  _staleTimer.unref();
...
function stop() {
  if (!_started) return;
  _started = false;
  try {  require('./heartbeat').stop();  } catch (e) { errReport.report("notifications", "require", e); }
  if (_staleTimer) { clearInterval(_staleTimer); _staleTimer = null; }
  try {  autoSupervisor.stop();  } catch (e) { errReport.report("notifications", "stop", e); }
  try {  ptyInjectionQueue.cancelAll();  } catch (e) { errReport.report("notifications", "cancelAll", e); }
...
async function _onTaskAvailable(uid) {
  ...
  if (!match) {
    ...
    if (tasks.length > 0) {
      ...
      } catch (e) { errReport.report("notifications", "map", e); }
  ...
  try {
    ...
    if (_frontendNotify) {
      for (const t of tasks) {
        ...
        } catch (e) { errReport.report("notifications", "getAgent", e); }
      }
...
async function _onTaskSubmitted({ task_id, sender_uid, receiver_uid, receiver_name, result }) {
  ...
  let match = null;
  try {  match = await wakeMod._findAnySessionByUid(sender_uid);  } catch (e) { errReport.report("notifications", "_findAnySessionByUid", e); }
  ...
  if (match && _frontendNotify) {
    try {
      _frontendNotify(match.id, 'idle', {
        ...
        result_preview: String(result || '').slice(0, 100),
        timestamp: new Date().toISOString(),
      });
    } catch (e) { errReport.report("notifications", "_frontendNotify", e); }
  }
  ...
  // Sprint 41: letter stays tiny (no result preview inline) + dedupe —
  // if a settlement notification for the same original task already sits
  // in PM's pending, skip creating another.
  try {
    const inbox = inboxStore.loadInboxSync(sender_uid);
    const exists = [...(inbox.pending || []), ...(inbox.in_progress || [])]
      .some((t) => t.reply_to === task_id && t.matched_via === 'settlement');
    if (exists) return;
    ...
    const notifTask = {
      ...
      content: `任务 #${task_id} 已被 ${receiver_name || 'worker'} 提交，请 settle_task approve/reject 审核结算。结果全文用 get_task_content(${task_id}, "result") 读取。`,
      priority: 'high',
      status: 'pending',
      reply_to: task_id,
    ...
    // Sprint 42 (user decision): settlement nudge — if the PM has not
    // responded to this settlement notification within 30 minutes,
    // force-inject check_inbox[BOOS] into the PM's PTY so they are
    // activated even if their loop was idle/offline.
    const SETTLEMENT_NUDGE_MS = 30 * 60 * 1000;
    setTimeout(async () => {
      try {
        const inbox = inboxStore.loadInboxSync(sender_uid);
        const stillPending = (inbox.pending || []).some((t) => t.task_id === notifTask.task_id);
        if (stillPending) {
          await ptyInjectionQueue.enqueue(sender_uid, 'check_inbox[BOOS]');
          console.log('[boos] settlement overdue — nudged PM', sender_uid.slice(-8), 'for', notifTask.task_id);
        }
      } catch (e) { errReport.report('notifications', 'settlement-nudge', e); }
    }, SETTLEMENT_NUDGE_MS).unref();
  } catch (e) {
  ...
  } catch (e) { errReport.report("notifications", "slice", e); }
}
...
async function _onTaskRejectedByPM({ task_id, sender_uid, receiver_uid, feedback }) {
  ...
  let match = null;
  try {  match = await wakeMod._findAnySessionByUid(receiver_uid);  } catch (e) { errReport.report("notifications", "_findAnySessionByUid", e); }
  ...
  // Sprint 41: feedback externalized to cache file; letter stays tiny + dedupe.
  try {
    ...
    const fbShaped = cacheStore.shapeContent(task_id, feedback || '', 'feedback');
    const notifTask = {
      task_id: store.genTaskId(),
      ...
      content: `任务 #${task_id} 被驳回，需修改后重新提交。反馈全文用 get_task_content(${task_id}, "feedback") 读取。`,
      priority: 'high',
      status: 'pending',
      reply_to: task_id,
      message_type: 'notification',
      required_capabilities: [],
      matched_via: 'settlement',
      metadata: { event: 'task_rejected_by_pm', task_id, feedback_ref: fbShaped.ref },
      created_at: new Date().toISOString(),
    };
  ...
  } catch (e) { errReport.report("notifications", "wakeAgent", e); }
}
...
async function _onTaskCompleted({ task_id, sender_uid, receiver_uid, receiver_name, result, metadata }) {
  ...
  if (sender_uid === ROOT_UID) {
    if (_frontendNotify) {
      try { _frontendNotify('__root__', 'idle', { type: 'root_task_completed', task_id, receiver_uid, receiver_name, result: String(result || '').slice(0, 100) }); } catch {}
    }
    try {
      const { notifyAgent } = require('./transport');
      notifyAgent(receiver_uid, 'notifications/agent_bus/root_response', {
        task_id, result_preview: String(result || '').slice(0, 200), receiver_uid, timestamp: new Date().toISOString(),
      });
    } catch (e) { errReport.report("notifications", "_frontendNotify", e); }
    return;
  }
    ...
    if (_frontendNotify) {
      try {
        if (match.status === 'running') {
          _frontendNotify(match.id, 'idle', { type: 'task_lifecycle', event: 'task_completed', task_id, receiver_uid, receiver_name, sender_uid, result_preview: String(result || '').slice(0, 100), timestamp: new Date().toISOString() });
  ...
  // Sprint 41: letter is a bare pointer — result text stays in the cache
  // file (get_task_content(task_id, "result")) + dedupe by reply_to.
  // Sprint 42 (user decision): never notify 'system' — it has no inbox
  // and the notice loops back into some agent's inbox via fallback routing.
  if (!sender_uid || sender_uid === 'system') return;
  try {
    const inbox = inboxStore.loadInboxSync(sender_uid);
    const exists = [...(inbox.pending || []), ...(inbox.in_progress || [])]
      .some((t) => t.reply_to === task_id && t.matched_via === 'notification');
    if (exists) return;
    ...
    const notifTask = {
      ...
      sender_intro: '任务完成通知',
      receiver_uid: sender_uid,
      content: `任务 #${task_id} 已完成。结果全文用 get_task_content(${task_id}, "result") 读取。`,
      priority: 'normal',
      status: 'pending',
      reply_to: task_id,
      message_type: 'notification',
