...
async function _onTaskAvailable(uid) {
  ...
  try {
    ...
    // Sprint 13.4: explicit action instruction — agent must respond_task after completing work.
    lines.push('[agent-bus] 📋 请处理以上任务，完成后用 respond_task 回复结果。');
...
async function wakeAgent(uid, opts = {}) {
  ...
  if (tasksToDeliver.length > 0) {
    ...
    const msgParts = [
      ...
      CORE_WAKE_INSTRUCTION,
      '',
      '─── 任务列表 ───',
    ];

    for (const t of tasksToDeliver) {
      const sender = t.sender_name || 'unknown';
      const statusTag = t.status === 'in_progress' ? '[已投递]' : '';
      const prio = t.priority === 'high' ? '🔴 高优' : t.priority === 'urgent' ? '⚡ 紧急' : '';
      const taskHeader = prio
        ? `[${prio}] ${statusTag} #${t.task_id} 来自 ${sender}:`
        : `[📨] ${statusTag} #${t.task_id} 来自 ${sender}:`;
      msgParts.push(taskHeader);
      msgParts.push(t.content);
      msgParts.push(''); // blank line between tasks
      // Mark pending tasks as in_progress. Already-in_progress tasks stay as is.
      if (t.status === 'pending') {
        try { store.updateTaskStatus(t.task_id, 'in_progress', null); } catch {}
      }
    }

    msgParts.push('─── 以上共 ' + tasksToDeliver.length + ' 个任务 ───');

    const fullMsg = msgParts.join('\n');
    try {
      _writeToPty(match.id, fullMsg);
    } catch (e) {
  ...
  const idleMsg = urgency === 'urgent'
    ? `\n[agent-bus] ⚡ ${customMsg || '紧急通知 — 请待命。'} (收件箱为空)\n`
    : `\n[agent-bus] 🔔 ${customMsg || '通知 — 当前无待处理任务，继续休眠。'}\n`;

  try {
    _writeToPty(match.id, idleMsg);
    if (_frontendNotify) {
