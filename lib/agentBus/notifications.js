...
async function _onTaskAvailable(uid) {
  ...
  if (!match) {
    if (agent.role === 'supervisor') {
      ...
      const attempts = wakeMod._wakeAttempts.get(uid) || 0;
      if (attempts < wakeMod.MAX_WAKE_ATTEMPTS) {
        wakeMod._wakeAttempts.set(uid, attempts + 1);
        console.log('[boos] wake-before-deliver: launching session for', agent.name, '(attempt ' + (attempts + 1) + ')');
        try {
          const { _internalLaunchAgentSession } = require('./handlersSession');
          await _internalLaunchAgentSession(uid, agent.name, agent.workspace);
          match = await wakeMod._findSessionByUid(uid);
        } catch (e) { console.warn('[boos] wake-before-deliver failed for', agent.name, e.message); }
      }
      if (!match) {
        wakeMod._logDeliveryFailure(uid, agent.name, 'no running BOOS session (wake attempts: ' + attempts + ')');
        if (attempts >= wakeMod.MAX_WAKE_ATTEMPTS) {
          console.warn('[boos] agent', agent.name, 'unreachable after', wakeMod.MAX_WAKE_ATTEMPTS, 'wake attempts');
          wakeMod._wakeAttempts.delete(uid);
        }
        return;
      }
      wakeMod._wakeAttempts.delete(uid);
