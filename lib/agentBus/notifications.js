...
async function _onTaskSubmitted({ task_id, sender_uid, receiver_uid, receiver_name, result }) {
  ...
  try {
    await store.insertTask({
      task_id: store.genTaskId(), sender_uid: receiver_uid,
      sender_name: receiver_name || 'worker', sender_intro: '任务已提交 — 等待 PM 审核结算',
      receiver_uid: sender_uid,
      content: `任务 #${task_id} 已被 ${receiver_name} 提交，请审核结算 (settle_task approve/reject): "${(result || '').slice(0, 80)}"`,
      priority: 'high', status: 'notification',
      reply_to: task_id, required_capabilities: [], matched_via: 'settlement',
  ...
  try {
    await wakeMod.wakeAgent(sender_uid, {
      ...
      message: `任务 #${task_id} 已被 ${receiver_name} 提交 — 需要 PM 审核结算`,
    });
  } catch {}
}
...
async function _onTaskRejectedByPM({ task_id, sender_uid, receiver_uid, feedback }) {
  ...
  try {
    await store.insertTask({
      task_id: store.genTaskId(), sender_uid: sender_uid,
      sender_name: 'PM', sender_intro: '任务驳回通知',
  ...
  try {
    await wakeMod.wakeAgent(receiver_uid, {
      urgency: 'normal',
      sender_name: 'PM',
      message: `任务 #${task_id} 被驳回: ${(feedback || '').slice(0, 50)}`,
    });
  } catch {}
}
