...
// ── PTY injection ──────────────────────────────────────────────────────
// Sprint 38: Single-shot write — command + \n in ONE pty.write() call.
// The old two-phase approach (text → delay → \r) no longer works because
// Claude Code's Ink TUI doesn't recognise a separately-written \r as
// "submit".  Writing the full command with \n in one write triggers
// Node.js readline's "line" event which Ink uses for input submission.
...
// Single-shot: write command + \n to PTY in one call.
// The \n triggers Node.js readline's "line" event → Ink submits the input.
