...
function _injectCommand(sessionId, command) {
  ...
  switch (mode) {
    case 'typed':
      // Sprint 38 vFinal: typed mode writes ONLY command chars (no trailing \r\n).
      // The \r submit is a separate write so ConPTY processes it as a distinct
      // Enter key event.
      _typedInject(sessionId, command, CHAR_DELAY_MS);
      setTimeout(() => write(sessionId, '\r'), CHAR_DELAY_MS);
      break;
    case 'paste':
      write(sessionId, PASTE_START + command + '\r' + PASTE_END);
      break;
    case 'burst':
    default:
      // Sprint 38 vFinal: Two separate pty.write() calls with a 50ms gap.
      //
      // On Windows ConPTY, writing "text\r\n" in ONE buffer causes ConPTY to
      // process the whole chunk atomically — the \r byte does NOT produce a
      // VK_RETURN key event; it's just UnicodeChar U+000D.  Ink sees a literal
      // CR character inside the input instead of an Enter keypress.
      //
      // Two back-to-back WriteFile calls can still merge inside ConPTY's input
      // buffer if the second arrives before the first is flushed.  A 50ms delay
      // between text and \r ensures ConPTY has processed the text batch and
      // flushed its KEY_EVENT queue before receiving the lone \r — which then
      // generates a clean VK_RETURN key-down event that Ink recognizes as Enter.
      //
      // This mirrors the human typing cadence through xterm.js: keystrokes
      // arrive as separate WebSocket messages → separate pty.write() calls,
      // each with natural inter-key gaps.
      write(sessionId, command);
      setTimeout(() => write(sessionId, '\r'), 50);
      break;
  }
}
