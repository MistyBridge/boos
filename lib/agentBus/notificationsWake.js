...
// ── PTY injection ──────────────────────────────────────────────────────
// Sprint 38 v2: Plain \r injection is the default. Paste-bracket
// (\x1b[200~...\x1b[201~) is opt-in via BOOS_PTY_PASTE_MODE=1 because
// Claude Code's Ink TUI does NOT process bracketed-paste sequences —
// commands wrapped in paste delimiters sit in the input bar unsubmitted.
//
// Runtime env detection (not module-load-time) so we can toggle without
// restarting the BOOS server.
...
// Runtime-checked injection mode: plain \r by default.
// Set BOOS_PTY_PASTE_MODE=1 to opt into bracketed-paste (may work with
// terminal emulators that support it — xterm.js does, Ink does not).
function _injectCommand(sessionId, command) {
  const usePaste = process.env.BOOS_PTY_PASTE_MODE === '1';  // opt-in, not default
  if (usePaste) {
    _writeToPty(sessionId, PASTE_START + command + '\r' + PASTE_END);
  } else {
    _writeToPty(sessionId, command + '\r');
  }
}
...
async function wakeAgent(uid, opts = {}) {
  ...
  // PTY — single-shot injection of command + \n (Sprint 38 fix).
  ...
  if (match) {
    ...
    if (term && !term.exitedAt) {
      try {
        ...
        _injectCommand(match.id, cmd);
        ptyWritten = true;
        console.log('[boos] wakeAgent: PTY injected to', agent.name,
...
async function compactAllWorkers(workspace, { milestone, note } = {}) {
  ...
  for (const w of workers) {
    ...
    try {
      _injectCommand(match.id, COMPACT_COMMAND);
