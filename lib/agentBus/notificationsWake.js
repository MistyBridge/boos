...
// ── PTY injection ──────────────────────────────────────────────────────
// Sprint 38 vFinal: Burst injection — `command + \r\n` written in a single
// pty.write() call. \r\n (CR+LF, 0x0D 0x0A) triggers submit in Claude Code's Ink TUI.
// \r alone in a separate write does nothing; \n alone moves cursor down.
// The combination \r\n in ONE write is what works.
// bracketed-paste (\x1b[200~...\x1b[201~) wrapper is broken in Ink.
//
// Modes (BOOS_PTY_INJECT_MODE env):
//   burst  — all chars in one write, \r\n-terminated (DEFAULT ✓)
...
function _buildWakeCommand(taskIds, opts = {}) {
  // Sprint 38 vFinal: Always use bare check_inbox[BOOS] — no context header.
  //
  // Why no context header?  Two \r submits in rapid succession race:
  // 1. First \r submits header line → Claude processes (even no-op text)
  // 2. While Claude is busy, second line +\r arrives → lost/buffered
  // Result: check_inbox[BOOS] sits in input buffer, never executes.
  //
  // Context (who submitted what) arrives through two OTHER channels:
  // - SSE: carrys sender_name, sender_uid, task_ids, message
  // - Inbox response: check_inbox returns full task with all metadata
  // PTY injection is ONLY the wake trigger — not a data channel.
  //
  // See memory: [[pty-injection-rules]] — \r\n triggers submit in Ink TUI
  return WAKE_COMMAND;
}
...
function _typedInject(sessionId, text, delayMs) {
  const write = module.exports._writeToPty || _writeToPty;
  let i = 0;
  const len = text.length;
  function next() {
    if (i >= len) return;
    write(sessionId, text[i]);
    i++;
...
function _injectCommand(sessionId, command) {
  ...
  // Defer to current _writeToPty binding (allows test interception via
  // monkey-patch of module.exports._writeToPty).
  const write = module.exports._writeToPty || _writeToPty;

  // All modes append \n\r (LF+CR) — standard terminal submit sequence.
  // \n (0x0A) moves cursor to next line, \r (0x0D) confirms submission.
  ...
  switch (mode) {
    case 'typed':
      ...
      break;
    case 'paste':
      write(sessionId, PASTE_START + fullText + PASTE_END);
      break;
    case 'burst':
    default:
      write(sessionId, fullText);
      break;
  }
}
...
// Sprint 38: auto-resumed session IDs — used by ptyInjectionQueue.drainIfIdle
// to skip queue-based injection; startup injection (sessionHelpers.js, 8s
// delay) handles the first check_inbox for newly-spawned sessions.
const _autoResumedIds = new Set();

function _wasAutoResumed(sessionId) { return _autoResumedIds.has(sessionId); }

// ── auto-resume mutex (Sprint 38 bugfix) ────────────────────────────────
// Serialize concurrent _autoResumeSession() calls.  When PM wakes 3 agents
// simultaneously and all 3 have dead PTYs, three spawnSessionRecord() calls
// race — each spawning a Claude process, each doing sync fs ops on .mcp.json,
// and each scheduling an 8s startup injection that collides with drainIfIdle's
// injection.  The mutex serializes the entire spawn+wait cycle so only one
// Claude starts at a time, preventing resource storms and injection races.
let _autoResumeRunning = false;
const _autoResumeWaiters = [];

async function _autoResumeSession(uid) {
  // Gate: only one auto-resume at a time.
  if (_autoResumeRunning) {
    await new Promise((resolve) => _autoResumeWaiters.push(resolve));
    if (_autoResumeRunning) {
      // Re-check: the waiter that ran before us may have already resumed
      // this agent's session.
      const alreadyResumed = await _findAnySessionByUid(uid);
      if (alreadyResumed && alreadyResumed.status === 'running') {
        const term = webTerminal.get(alreadyResumed.id);
        if (term && !term.exitedAt) return alreadyResumed;
      }
    }
  }
  _autoResumeRunning = true;

  try {
    return await _autoResumeSessionInner(uid);
  } finally {
    _autoResumeRunning = false;
    // Release next waiter.
    const next = _autoResumeWaiters.shift();
    if (next) next();
  }
}

async function _autoResumeSessionInner(uid) {
  ...
  try {
    ...
    // Sprint 38 bugfix: pass skipStartupInjection=true so sessionHelpers'
    // 8s setTimeout injection is suppressed.  drainIfIdle() injects
    // immediately after we return — one clean injection, no race.
    await spawnFn({ record: session, cli, cfg, body: {}, resume: true, skipStartupInjection: true });
    // Wait for Claude Code to fully initialise (MCP, agent-bus SSE, Ink TUI).
    // Must match sessionHelpers.js startup injection delay (8s) so PTY is
    // ready when check_inbox arrives.  Formerly 1.5s — caused injection to
    // hit an uninitialised TUI, leaving text in the input box.
    await new Promise((r) => setTimeout(r, 8000));
    const term = webTerminal.get(session.id);
    if (term && !term.exitedAt) {
      _autoResumedIds.add(session.id);
      // Clean up after 60s — long enough for drainIfIdle to consume the flag.
      setTimeout(() => _autoResumedIds.delete(session.id), 60_000);
...
async function wakeAgent(uid, opts = {}) {
  ...
  // Sprint 42: boundary marker — the wake path ends in native pty writes.
  try { require('../crashForensics').note('wakeAgent', { uid: String(uid).slice(-8), name: agent.name }); } catch {}
...
module.exports = {
  ...
  setFrontendNotify,
  _buildWakeCommand, _findSessionByUid, _findAnySessionByUid, _autoResumeSession,
  _wasAutoResumed,
