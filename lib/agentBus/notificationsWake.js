...
// Build a context-aware wake command: check_inbox[BOOS, <task_id>]
// so the agent sees in their terminal exactly which task triggered the wake.
function _buildWakeCommand(taskIds) {
  if (taskIds && taskIds.length > 0) {
    return 'check_inbox[BOOS, ' + taskIds[0] + ']';
  }
  return WAKE_COMMAND;
}
const PTY_SUBMIT_DELAY_MS = parseInt(process.env.BOOS_PTY_SUBMIT_DELAY_MS, 10) || 30;
...
async function _findSessionByUid(uid) {
  // Sprint 33: uid = cliSessionId (Claude UUID). Find session directly.
  ...
  // Legacy: check agentUid field on session records (old agent_xxx).
...
// Find ANY session for the agent (running or stopped), preferring running.
async function _findAnySessionByUid(uid) {
  // Sprint 33: uid = cliSessionId.
  const session = await persistedSessions.findByCliSessionId(uid);
  if (session) return session;
  // Legacy: check agentUid field.
  const all = await persistedSessions.loadAll();
  return all.find((s) => s.agentUid === uid && !s.deletedAt) || null;
}

// Auto-resume a stopped session so the agent becomes reachable via PTY.
...
async function wakeAgent(uid, opts = {}) {
  ...
  // Sprint 35: read from per-agent inbox file (tiny ~few KB) instead of
  // scanning the shared 1.16 MB agent-bus.json.
  ...
  // SSE notification — best-effort, may fail silently on half-open connections.
  ...
  // PTY — always attempted in PARALLEL with SSE, not gated behind SSE failure.
  // Reason: SSE notifyAgent() returns true even on half-open TCP connections
  // where the client never actually receives the event. PTY is the reliable
  // fallback that injects directly into the terminal.
  let ptyWritten = false;
  let match = await _findSessionByUid(uid);

  // Auto-resume: when no live PTY session exists, try to bring one up.
  if (!match) {
  ...
  if (match) {
    ...
    if (term && !term.exitedAt) {
      try {
        ...
        await _injectCommand(match.id, cmd);
        ptyWritten = true;
        console.log('[boos] wakeAgent: PTY submit injected to', agent.name,
          '(' + tasksToDeliver.length, 'pending tasks) cmd:', cmd);
      } catch (e) {
