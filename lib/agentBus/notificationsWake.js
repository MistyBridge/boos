...
function _injectCommand(sessionId, command) {
  ...
  switch (mode) {
    case 'typed':
      ...
      // Enter key event.  Avoiding \n after \r prevents a spurious line-feed
      // from landing in the next input buffer after submit.
      _typedInject(sessionId, command, CHAR_DELAY_MS);
      setTimeout(() => _writeToPty(sessionId, '\r'), CHAR_DELAY_MS);
      break;
    case 'paste':
      _writeToPty(sessionId, PASTE_START + command + '\r' + PASTE_END);
      break;
    case 'burst':
    default:
      // Sprint 38 vFinal: Two separate pty.write() calls.
      //
      ...
      //
      // Splitting into two writes (text, then \r alone) mirrors what xterm.js
      // does when a human types: every keystroke is its own WebSocket message →
      // its own pty.write() call.  The second write carries ONLY \r so ConPTY
      // generates a KEY_EVENT with wVirtualKeyCode=VK_RETURN.
