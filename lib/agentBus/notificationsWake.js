...
// ── PTY injection ──────────────────────────────────────────────────────
// Sprint 38: Single-shot write — command + \r in ONE pty.write() call.
// Ink's TextInput in raw mode treats \r (0x0D, Carriage Return) as the
// Enter/Return key. The old two-phase approach (text → delay → \r in
// separate call) broke because Ink needs the \r in the same input burst.
// \n (0x0A) is Ctrl+J and does NOT trigger submit in Ink.
...
// Single-shot: write command + \r to PTY in one call.
// \r triggers Ink's "return" key handler → submits the input.
