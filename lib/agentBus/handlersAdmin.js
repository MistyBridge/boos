...
async function _activateWorkflow(args, ctx) {
  ...
  const dispatchFn = async (content, requiredCapabilities, workspace, workflowId, stageId) => {
    ...
    if (r.ok) {
      try {  await store.setTaskWorkflowMeta(taskId, workflowId, stageId);  } catch (e) { errReport.report("handlersAdmin", "setTaskWorkflowMeta", e); }
    }
...
async function _requestDecision(args, ctx) {
  ...
  } catch (e) { errReport.report("handlersAdmin", "oper", e); }
...
async function _escalateToPM(args, ctx, myself) {
  ...
  } catch (e) { errReport.report("handlersAdmin", "resolveProjectPM", e); }
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (args.blocking_task_id) {
        ...
        } catch (e) { errReport.report("handlersAdmin", "blockTask", e); }
      }
...
async function _purgeWorkspace(args, ctx) {
  // Sprint 42: supervisor-scoped workspace purge with auto-snapshot.
  const supErr = _requireSupervisor(ctx);
  if (supErr) return supErr;
  const snap = registry.snapshot();
  if (!snap.ok) return { error: 'snapshot failed: ' + snap.error };
  const result = await registry.purgeWorkspace(args.workspace);
  return { ...result, snapshot: snap.snapshot };
}
...
async function _killWorker(args, ctx) {
  ...
  } catch (e) { errReport.report("handlersAdmin", "releaseAllForAgent", e); }
...
async function _listAgentCards(args, ctx) {
  ...
  } catch (e) { errReport.report("handlersAdmin", "canonical", e); }
  ...
  } catch (e) { errReport.report("handlersAdmin", "filter", e); }
  ...
  const cards = agents.map((a) => {
    ...
    } catch (e) { errReport.report("handlersAdmin", "getMyDagTasks", e); }
...
async function _teamMatrix(args, ctx) {
  ...
  // Live PTY detection.
  let livePtyIds = new Set();
  try {  const wt = require('../webTerminal'); livePtyIds = new Set((wt.list ? wt.list() : []).filter((t) => !t.exitedAt).map((t) => t.id));  } catch (e) { errReport.report("handlersAdmin", "list", e); }
  ...
  } catch (e) { errReport.report("handlersAdmin", "push", e); }
...
async function _boosTerminalList(_args, ctx) {
  ...
  try {
    ...
    if (wt && typeof wt.list === 'function') {
      ...
      } catch (e) { errReport.report("handlersAdmin", "set", e); }
...
async function _wakeAgent(args, ctx) {
  ...
  } catch (e) { errReport.report("handlersAdmin", "getAgent", e); }
...
module.exports = {
  ...
  _requestDecision,
  _assignTask, _listAllAgents, _killWorker, _purgeWorkspace,
