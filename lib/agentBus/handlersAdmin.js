...
async function _boosTerminalList(_args, ctx) {
  ...
  const agent = auth.getAgent(ctx.uid);
  const myWorkspace = agent?.workspace || ctx.workspace;
  ...
  try {
    ...
    if (wt && typeof wt.list === 'function') {
      ...
      for (const t of rawList) {
        ...
        // Filter: only show terminals in our workspace.
        if (myWorkspace && persisted.workspace && persisted.workspace !== myWorkspace) continue;
        terminals.push({
