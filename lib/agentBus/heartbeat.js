...
async function touch(agentUid) {
  ...
  } catch (e) { errReport.report("heartbeat", "_schedule", e); }
}
...
async function _onTimeout(uid) {
  ...
  if (_onUnresponsive) {
    try {  _onUnresponsive(uid, agent.name, count);  } catch (e) { errReport.report("heartbeat", "_onUnresponsive", e); }
  }
}
...
async function _reassignTasks(uid, agentName) {
  ...
  try {
    ...
    for (const t of inbox.in_progress) {
      ...
      // Sprint 39: NEVER reassign tasks awaiting PM settlement.
      // respond_task sets status='submitted' in-place (inboxStore.updateTask
      // uses Object.assign without moving arrays).  If the agent then
      // goes unresponsive before PM settles, reassigning here would
      // revert status to 'pending' and the PM can never approve/reject.
