...
function start(storeRef, opts = {}) {
  ...
  // Schedule timeouts for all existing agents with staggered delays.
  // Without staggering, agents whose last_seen_at is older than the
  // threshold would fire all timeouts simultaneously on startup — that
  // floods withFileLock with N concurrent writes and can exhaust event-loop
  // resources.  We add 0–30s of jitter per agent so the bursts spread out.
  try {
    const agents = _store.listAllAgents();
    let i = 0;
    for (const a of agents) {
      const delay = Math.min((i++ % 10) * 3000, UNRESPONSIVE_THRESHOLD_MS); // 0–27s jitter
      setTimeout(() => _schedule(a.uid), delay).unref();
    }
  } catch {}

  console.log('[heartbeat] event-driven scanner active (threshold=' + Math.round(UNRESPONSIVE_THRESHOLD_MS / 1000) + 's, per-agent timeouts, zero polling, staggered startup)');
}
