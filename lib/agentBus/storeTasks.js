...
function _findTaskInbox(taskId) {
  ...
  } catch (e) { errReport.report("storeTasks", "getTaskSync", e); }
  return null;
}
...
async function _findTaskInboxAsync(taskId) {
  ...
  } catch (e) { errReport.report("storeTasks", "getTask", e); }
  return null;
}
...
const _taskOps = {
  ...
  listActiveTasks(uid) {
    ...
    } catch (e) { errReport.report("storeTasks", "loadInboxSync", e); }
    return [];
  },
  ...
  // NOTE: NOT async. The body is fully synchronous (readdirSync + loadInboxSync),
  // and constraints.checkLimits() calls this without await (it's sync code).
  // Await-ing a non-Promise still works at the await sites, so removing the
  // async keyword is safe for both sync and async callers.
  listMyTasks(uid) {
    const all = [];
    try {
      const fs = require('fs');
      ...
      for (const f of files) {
        ...
        } catch (e) { errReport.report("storeTasks", "push", e); }
      }
    } catch (e) { errReport.report("storeTasks", "push", e); }
...
function listAllPendingQueues() {
  ...
  try {
    ...
    for (const f of files) {
      ...
      } catch (e) { errReport.report("storeTasks", "add", e); }
    }
  } catch (e) { errReport.report("storeTasks", "add", e); }
...
function findTask(taskId) {
  ...
  } catch (e) { errReport.report("storeTasks", "findIndex", e); }
...
async function pruneOldTasks(olderThanMs = 7 * 24 * 3600_000) {
  ...
  try {
    ...
    for (const f of files) {
      ...
      } catch (e) { errReport.report("storeTasks", "saveInbox", e); }
    }
  } catch (e) { errReport.report("storeTasks", "saveInbox", e); }
  return totalPruned;
}
