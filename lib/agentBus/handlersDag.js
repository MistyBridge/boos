...
async function _dagActivate(args, ctx) {
  ...
  const result = await dagEngine.activateWithReport(args.dag_id);

  // Sprint 38 fix: dispatch ready tasks to executors via agent-bus + wake.
  // Without this, DAG activation doesn't notify executors — they never
  // receive their tasks and the DAG stalls immediately.
  if (result && result.ready_tasks && result.ready_tasks.length > 0) {
    try {
      const queue = require('./queue');
      const notifications = require('./notifications');
      for (const rt of result.ready_tasks) {
        if (!rt.executor_uid) continue;
        try {
          await queue.sendTask({
            sender: { uid: ctx.uid, name: ctx.name || 'PM', intro: 'DAG task dispatch.' },
            receiver_uid: rt.executor_uid,
            content: `[DAG TASK] "${rt.title}" (${rt.task_id})\n` +
              `DAG activated. You are assigned as executor.\n` +
              `Use dag_submit_task to submit your work.`,
            priority: 'high',
            metadata: { type: 'dag_dispatch', task_id: rt.task_id, dag_id: args.dag_id },
          });
          try { await notifications.wakeAgent(rt.executor_uid, { urgency: 'urgent', sender_name: 'PM' }); } catch {}
        } catch (e) { errReport.report("handlersDag", "wakeAgent", e); }
      }
    } catch (e) { errReport.report("handlersDag", "wakeAgent", e); }
  }
  ...
  if (result.ok) {
    ...
    try {
      ...
      if (cascade && cascade.newly_ready_tasks && cascade.newly_ready_tasks.length > 0) {
        ...
        // Sprint 38 fix: dispatch newly-ready tasks to executors.
        try {
          const queue = require('./queue');
          const notifications = require('./notifications');
          const dagStore = require('./dagStore');
          for (const taskId of cascade.newly_ready_tasks) {
            const t = dagStore.getTask(taskId);
            if (!t || !t.executor_uid) continue;
            try {
              await queue.sendTask({
                sender: { uid: ctx.uid, name: ctx.name || 'Reviewer', intro: 'DAG task unlocked.' },
                receiver_uid: t.executor_uid,
                content: `[DAG UNLOCKED] "${t.title}" (${t.task_id})\n` +
                  `Dependencies approved. You can now execute this task.\n` +
                  `Use dag_submit_task to submit your work.`,
                priority: 'high',
                metadata: { type: 'dag_dispatch', task_id: t.task_id, dag_id: t.dag_id },
              });
              try { await notifications.wakeAgent(t.executor_uid, { urgency: 'urgent', sender_name: 'PM' }); } catch {}
            } catch (e) { errReport.report("handlersDag", "wakeAgent", e); }
          }
        } catch (e) { errReport.report("handlersDag", "wakeAgent", e); }
      }
    ...
    } catch (e) { errReport.report("handlersDag", "afterDagCompleted", e); }
  }
  return result;
}
...
async function _dagRejectTask(args, ctx) {
  ...
  if (result.ok) {
    ...
    if (result.escalated) {
      ...
      } catch (e) { errReport.report("handlersDag", "onTaskEscalated", e); }
    }
  }
  return result;
}
...
async function _goalCreate(args, ctx) {
  ...
  if (result.ok) {
    ...
    } catch (e) { errReport.report("handlersDag", "sendFeedback", e); }
  }
...
async function _dagProposeTask(args, ctx) {
  ...
  try {
    ...
    if (args.blocking_task_id) {
      ...
      } catch (e) { errReport.report("handlersDag", "blockTask", e); }
    }
    ...
    } catch (e) { errReport.report("handlersDag", "slice", e); }
