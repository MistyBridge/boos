...
async function _dagProposeTask(args, ctx) {
  ...
  try {
    ...
    return { ok: true, task_id: taskId, status: 'proposed', blocking_task_id: args.blocking_task_id || null };
  } catch (e) {
    return { error: e.message };
  }
}
...
async function _dagApproveProposal(args, ctx) {
  ...
  const dagStore = require('./dagStore');
  try {
    return await dagStore.approveProposal(args.task_id, {
...
async function _dagRejectProposal(args, ctx) {
  ...
  const dagStore = require('./dagStore');
  try {
    return await dagStore.rejectProposal(args.task_id, args.reason);
  } catch (e) {
    return { error: e.message };
  }
}
