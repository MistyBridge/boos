...
const TOOLS = [
  {
    name: 'register_agent',
    description:
      'Register this agent in a workspace. Sprint 42: identity is AUTO-DETECTED ' +
      'from BOOS_CLI_SESSION_ID (no cli_session_id needed) and agents are ' +
      'auto-registered on first connect. Calling this only enriches metadata ' +
      '(name/role/capabilities/intro). IDEMPOTENT: same uid → same persistent identity.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        workspace: { type: 'string', description: 'Workspace name. Worker 只能选择已存在的工作区；仅 PM(supervisor) 可新建工作区 (Sprint 42 约束)。' },
        ...
        cli_session_id: { type: 'string', description: 'Optional override (normally auto-detected): your Claude --resume UUID.' },
      },
  ...
  },
  {
    ...
    description:
      ...
      'Source tagging: BOOS system injections appear as check_inbox[BOOS] in your terminal; ' +
      'your own calls return source="check_inbox[<your-uid>]" in the response.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description: 'Read the FULL externalized content of a task on demand. Long task bodies, submission results, and PM feedback are stored in per-task cache files — letters carry only a short summary. Call this when a letter references cache/… or says "全文用 get_task_content 读取". Kind: content (task body) | result (submission) | feedback (PM rejection feedback).',
    inputSchema: {
      type: 'object',
      properties: {
        task_id: { type: 'string', description: 'Task ID whose full content to read' },
        kind: { type: 'string', enum: ['content', 'result', 'feedback'], description: 'Which stored content to read (default: content)' },
      },
      required: ['task_id'],
    },
  },
  ...
  },
  {
    ...
    description:
      'Supervisor-only. Remove ALL agents registered under a workspace ' +
      '(identity cards + indexes). Takes a rotating snapshot first so the ' +
      'purge is rollback-able. Agents will re-register on reconnect — use ' +
      'only to retire an old workspace.',
    inputSchema: {
      type: 'object',
      properties: {
        workspace: { type: 'string', description: 'Workspace name to purge entirely.' },
      },
      required: ['workspace'],
    },
  },
