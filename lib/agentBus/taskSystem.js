...
// Sprint 36: delegate to unified auth module (takes ctx.uid, not bare string).
function _requirePMorPMO(callerUid) {
  const err = auth.requirePMorPMO({ uid: callerUid });
  if (err) {
    throw new Error(err.error || 'Permission denied: PM (supervisor) or PMO role required');
  }
}
