...
async function interruptTask(taskId, requesterUid) {
  const ownerUid = _findTaskOwner(taskId);
  if (!ownerUid) return { ok: false, error: 'task not found' };
  const r = await inboxStore.interruptTask(ownerUid, taskId, true); // recycle to pending
  return r;
}
...
async function respondTask(taskId, requesterUid, result, metadata) {
  ...
  const needsSettlement = responderRole !== 'supervisor'
  ...
  const r = await inboxStore.completeTask(ownerUid, taskId, ST_COMPLETED, result || '');
  if (!r.ok) return r;
  _unindexTask(taskId);
...
async function settleTask(taskId, approverUid, action, feedback) {
  ...
  const ownerUid = _findTaskOwner(taskId);
  if (!ownerUid) return { ok: false, error: 'task not found in active inboxes' };
  const task = await inboxStore.getTask(ownerUid, taskId);
  if (!task) return { ok: false, error: 'task not found' };
...
async function retryTask(taskId, requesterUid, isSupervisor = false) {
  ...
  // Retry requires reading from archive (archived completed/cancelled task).
  // For now, keep simple: the task must still be in the inbox (pending or in_progress).
  const ownerUid = _findTaskOwner(taskId);
  if (!ownerUid) return { ok: false, error: 'task not found in active inboxes (may be archived)' };
  const task = await inboxStore.getTask(ownerUid, taskId);
  if (!task) return { ok: false, error: 'task not found' };
