...
async function sendTask({ task_id, sender, receiver_uid, content, priority, reply_to, required_capabilities, metadata }) {
...
async function checkInbox(uid) {
  ...
  // A3: emit task_claimed lifecycle event for SSE push to frontend.
...
async function respondTask(taskId, requesterUid, result, metadata) {
  ...
  await store.updateTaskStatus(taskId, ST_COMPLETED, result || '', metadata || null);
