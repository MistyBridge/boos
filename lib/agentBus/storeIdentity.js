...
// Sprint 42: connection lifecycle == PTY lifecycle. transport.js registers a
// handler so that when a PTY exits, its agent's SSE connection is torn down
// too (and vice-versa on reconnect the header re-binds identity).
let _agentOfflineHandler = null;
function setAgentOfflineHandler(fn) { _agentOfflineHandler = fn; }
...
async function rebuildAllIndices() {
  return withFileLock(DB_PATH, async () => {
    ...
    let mcpFromPg = 0;
    try {
      const { query } = require('../sqliteStore');
      const r = await query(
...
async function onSessionExited(sessionId) {
  ...
  } catch (e) { errReport.report("storeIdentity", "onSessionExited", e); }
  // JSON identity card no longer stores pty_pid — nothing to clear.
  // Sprint 42: PTY exited → notify transport to close this agent's SSE
  // connection (connection lifecycle is bound to PTY lifecycle).
  if (_agentOfflineHandler) {
    try {
      const { get } = require('../persistedSessions');
      const rec = await get(sessionId);
      const uid = rec && rec.cliSessionId;
      if (uid) _agentOfflineHandler(uid, sessionId);
    } catch (e) { errReport.report("storeIdentity", "onSessionExited-offline", e, { sessionId }); }
  }
}
...
async function linkIdentityToSession(agentUid, sessionId, cwd, ptyPid) {
  ...
  } catch (e) { errReport.report("storeIdentity", "linkSession", e); }
}
...
async function bindMcpSession(mcpSessionId, agentUid) {
  ...
  } catch (e) { errReport.report("storeIdentity", "upsert", e); }
  return withFileLock(DB_PATH, async () => {
    const db = await _load();
...
function autoResolveIdentity(mcpSessionId) {
  ...
  // SQLite authoritative fallback: when JSON index is stale (agent reconnected
...
function _pgLookupMcp(queryFn, mcpSessionId) {
  ...
  // The SQLite query runs synchronously (node:sqlite DatabaseSync).
  // _pgLookupMcp checks the sqlite in-memory cache of the identity_index.
  try {
    const pg = require('../sqliteStore');
  ...
  } catch (e) { errReport.report("storeIdentity", "oper", e); }
  return null;
}
...
// ── Sprint 42: duplicate identity cleanup ──────────────────────────────
// Session rebuilds (new Claude session → new UUID) leave stale duplicate
// cards with the same name+workspace. Keep the most recently seen card,
// drop the rest. System cards (root, workspace '*') are never touched.

async function dedupeIdentities() {
  return withFileLock(DB_PATH, async () => {
    const db = await _load();
    const byKey = new Map();   // name|workspace → uid
    let removed = 0;
    for (const [uid, agent] of Object.entries(db.agents || {})) {
      ...
      if (!existing) { byKey.set(key, uid); continue; }
      const cur = db.agents[existing];
      const keep = (agent.last_seen_at || '') >= (cur.last_seen_at || '') ? uid : existing;
      const drop = keep === uid ? existing : uid;
      delete db.agents[drop];
      if (db.identities) delete db.identities[drop];
      if (db.identity_by_name_ws) {
        for (const k of Object.keys(db.identity_by_name_ws)) {
          if (db.identity_by_name_ws[k] === drop) delete db.identity_by_name_ws[k];
        }
      }
      byKey.set(key, keep);
      removed++;
    ...
    if (removed > 0) {
      await _save(db);
      console.log('[agent-bus] dedupeIdentities: removed', removed, 'duplicate card(s)');
    }
    return removed;
  });
}
...
module.exports = {
  ...
  setAgentOfflineHandler, dedupeIdentities,
