...
async function _allAgentIds(uid) {
  const ids = new Set([uid]);

  // ── Primary path: IdentityResolver.expand() covers ALL ID types ──
  try {
    const resolver = getResolver();
    for (const id of await resolver.expand(uid)) ids.add(id);
    return ids;
  } catch { /* resolver unavailable → fallback below */ }

  // ── Fallback: manual lookups (only when resolver is unavailable) ──
...
async function _isPM(uid) {
  try {
    ...
    if (folderId) {
      ...
      for (const id of await _allAgentIds(uid)) {
        if (levels[id] && (levels[id].sandbox === 'PM' || levels[id].sandbox === 'PMO')) return true;
      }
    }
    return false;
  } catch {
...
// Resolve an agent's folder ID.
//
// Sprint 33: uid IS the cliSessionId (Claude UUID). Direct lookup.
async function _resolveFolderId(agentUid) {
  try {
    // ── Sprint 33 Fast path: uid = cliSessionId, find session directly ──
    const session = await persistedSessions.findByCliSessionId(agentUid);
    if (session?.folderId) return session.folderId;
    if (session) return null; // session exists but no folder bound

    // ── Legacy: IdentityResolver canonical resolution (old agent_xxx → session) ──
    try {
      const resolver = getResolver();
      const boosId = await resolver.canonical(agentUid);
      if (boosId) {
    ...
    // ── Legacy Path 1: transport session ID from agent-bus sessions table ──
    const sessionId = store.getSessionByAgentUid(agentUid);
    if (sessionId) {
      const session = await persistedSessions.findById(sessionId);
      if (session?.folderId) return session.folderId;
    }
    // ── Legacy Path 2: BOOS session ID from identity card ──
    ...
    // ── Legacy Path 3: fallback — match agent name to BOOS session ──
      ...
      if (match?.folderId) {
        ...
        try {
          await store.writeIdentity(agentUid, {
            cli_session_id: match.cliSessionId || undefined,
...
async function getWritePermission(agentUid) {
  try {
    ...
    const levels = await folders.getAgentLevels(folderId);
    for (const id of await _allAgentIds(agentUid)) {
...
async function getFilesystemMcpConfig(opts = {}) {
  ...
  // PM/supervisor always gets full project access + unrestricted write.
  if (agentUid && await _isPM(agentUid)) {
    return { command: 'node', args: [FS_SERVER, PROJECT_ROOT] };
  }

  const args = [FS_SERVER];
  let rootPath = PROJECT_ROOT;

  // Non-PM agent with a bound folder rootPath → sandbox to that path.
  if (folderId) {
    const rp = await folders.getRootPath(folderId);
    if (rp) rootPath = path.resolve(rp);
  }

  args.push(rootPath);

  // Sprint 32: Write permission enforcement — when agent has write:false
  // (set via FolderSettingsModal "代码写入" checkbox), restrict the MCP
  // filesystem server to document-only extensions.
  if (agentUid) {
    try {
      const hasWrite = await getWritePermission(agentUid);
      if (!hasWrite) {
        args.push('--write-extensions=.md,.markdown,.txt,.json,.yaml,.yml,.toml');
      }
    } catch { /* best-effort */ }
  }
