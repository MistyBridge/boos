  ...
  try {
    ...
    await pool.query(
      `INSERT INTO identity_index (cli_session_id, agent_name, workspace, role, capabilities, mcp_session_id, pty_pid, cwd, updated_at)
       VALUES ($1, COALESCE($2, $1), COALESCE($3, 'boos'), COALESCE($4, 'worker'), $5, $6, $7, $8, NOW())
...
async function linkSession(uuid, sessionId, cwd) {
  ...
  try {
    ...
    if (cwd) {
      await pool.query(
        'UPDATE identity_index SET cwd = $1, updated_at = NOW() WHERE cli_session_id = $2',
        [cwd, uuid]
      );
    }
  } catch (e) {
...
async function onSessionExited(sessionId) {
  ...
  try {
    await pool.query(
      `UPDATE identity_index SET pty_pid = 0, updated_at = NOW()
...
async function syncFromJson() {
  ...
  try {
    ...
    if (count > 0) {
      console.log('[identityAdapter] synced', count, 'UUID identities from JSON → PG');
    }
  } catch (e) {
