...
function createSessionHelpers({
  ...
  async function spawnSessionRecord({
    ...
    // Sprint 38 v2: Plain \r by default. Paste-bracket opt-in via
    // BOOS_PTY_PASTE_MODE=1. Runtime env check (not module-load-time).
    if (isManaged) {
      setTimeout(() => {
        try {
          const usePaste = process.env.BOOS_PTY_PASTE_MODE === '1';  // opt-in
          const cmd = usePaste
            ? '\x1b[200~check_inbox[BOOS]\r\x1b[201~'
            : 'check_inbox[BOOS]\r';
          webTerminal.write(record.id, cmd);
          console.log('[boos] startup injection: check_inbox sent to session', record.id,
            usePaste ? '(paste-bracket)' : '(plain \\r)');
        } catch (e) {
