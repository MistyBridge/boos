...
function createSessionHelpers({
  ...
  async function spawnSessionRecord({
    ...
    // Sprint 17: Startup injection — inject check_inbox for managed agents.
    // CC starts in REPL mode; without injection, agent never enters the
    // SSE blocking loop and never receives tasks.
    ...
    if (isManaged) {
      setTimeout(() => {
        try {
          webTerminal.write(record.id, 'check_inbox(wait=true)\n');
