...
function createSessionHelpers({
  ...
  async function spawnSessionRecord({
    ...
    // Sprint 38: Single-shot write (command + \r) — \r triggers Ink's
    // Enter/Return key handler. Must be in the same pty.write() call.
    if (isManaged) {
      setTimeout(() => {
        try {
          webTerminal.write(record.id, 'check_inbox[BOOS]\r');
