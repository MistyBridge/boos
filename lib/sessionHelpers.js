...
function createSessionHelpers({
  ...
  async function spawnSessionRecord({
    ...
    // Sprint 38 v3: Burst injection is the default (command+\r in one write).
    // Paste-bracket and typed are opt-in via BOOS_PTY_INJECT_MODE.
    ...
    if (isManaged) {
      setTimeout(() => {
        try {
          ...
          const text = 'check_inbox[BOOS]\r';
          if (mode === 'paste') {
            webTerminal.write(record.id, '\x1b[200~' + text + '\x1b[201~');
          } else if (mode === 'typed') {
            const delay = parseInt(process.env.BOOS_PTY_CHAR_DELAY_MS, 10) || 8;
            let i = 0;
            function next() {
              if (i >= text.length) return;
              webTerminal.write(record.id, text[i]);
              i++;
              if (i < text.length) setTimeout(next, delay);
            }
            next();
          } else {
            webTerminal.write(record.id, text);
          }
