...
function createSessionHelpers({
  ...
  async function spawnSessionRecord({
    ...
    // Event-driven activation: CC starts in REPL mode; without injection,
    // agent never checks inbox and never receives tasks.
    if (isManaged) {
      setTimeout(async () => {
        try {
          // Two-phase submit: type the command, then a SEPARATE CR (0x0D).
          webTerminal.write(record.id, 'check_inbox[BOOS]');
          await new Promise((r) => setTimeout(r, 120));
          webTerminal.write(record.id, '\r');
      ...
      }, 5000); // Wait for CC boot (read CLAUDE.md, connect MCPs, show prompt)
    }
