...
function createSessionHelpers({
  ...
  async function spawnSessionRecord({
    ...
    // The old two-phase approach (text → delay → \r) doesn't trigger
    // submit in Claude Code's Ink TUI.  Appending \n to the command text
    // and writing it all in one shot triggers readline's "line" event.
    if (isManaged) {
      setTimeout(() => {
        try {
