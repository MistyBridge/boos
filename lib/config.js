...
// ── Single source of truth for the default listen port ─────────
// Every module that needs a port should reference this constant or
// call getRuntimePort() to get the actual bound port at runtime.
const DEFAULT_PORT = 7780;

// Runtime port state — set by server.js once the port is bound.
// Agent-bus modules that cannot import server.js use getRuntimePort().
let _runtimePort = DEFAULT_PORT;

function setRuntimePort(port) {
  _runtimePort = Number(port) || DEFAULT_PORT;
}
