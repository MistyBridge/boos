...
2. SSE 推送 + auto-wake → check_inbox 取任务 (事件驱动，无轮询)
...
function _mcpConfig(agentBusUrl) {
  return {
    mcpServers: {
      ...
      'agent-bus': {
        type: 'sse',
        url: agentBusUrl || ('http://127.0.0.1:' + getRuntimePort() + '/mcp/sse'),
      },
    },
  };
}
...
function listAvailableRoles() {
  ...
  } catch (e) { errReport.report("hrAgent", "push", e); }
...
async function _createBoosSession(agentName, cwd) {
  ...
  } catch (e) { errReport.report("hrAgent", "oper", e); }

  try {  findCliById = require('./cliHelpers').findCliById;  } catch (e) { errReport.report("hrAgent", "require", e); }
  try {  spawnSessionRecord = require('./sessionHelpers').getSpawnSessionRecord();  } catch (e) { errReport.report("hrAgent", "require", e); }
  try {  folders = require('./folders');  } catch (e) { errReport.report("hrAgent", "require", e); }
  ...
  if (folders) {
    ...
    } catch (e) { errReport.report("hrAgent", "create", e); }
  }
  ...
  } catch (e) { errReport.report("hrAgent", "save", e); }
  ...
  if (spawnSessionRecord) {
    ...
    } catch (e) { errReport.report("hrAgent", "markRunning", e); }
  }

  return sessionId;
}

// ── Capability → role name lookup ─────────────────────────────────────────
// Builds a reverse map from capability tag to human-readable role title,
// derived from BUILTIN_ROLES. Used by notifications.js for recruitment
// suggestions so it doesn't hardcode the mapping.

let _capNameMap = null;
function _buildCapNameMap() {
  if (_capNameMap) return _capNameMap;
  _capNameMap = {};
  for (const role of BUILTIN_ROLES) {
    for (const cap of role.capabilities) {
      _capNameMap[cap] = role.title;
    }
  }
  return _capNameMap;
}
...
function getCapabilityName(capability) {
  return _buildCapNameMap()[capability] || null;
}

module.exports = { listAvailableRoles, handleRecruitRequest, getCapabilityName };
