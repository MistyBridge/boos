...
async function reclaimPortFromOldInstance(preferredPort) {
    ...
    if (portInUse) {
      ...
      try {
        ...
        if (match) {
          ...
          if (isBoos) { oldPid = detectedPid; console.log(`[boos] 检测到端口 ${preferredPort} 被 BOOS 进程 PID ${oldPid} 占用`); }
          else { console.warn(`[boos] 端口 ${preferredPort} 被非 BOOS 进程占用 — 跳过清理`); oldPort = null; }
        }
  ...
  if (oldPid && !isPidDead(oldPid)) {
    ...
    try {
      await new Promise((resolve) => {
        const req = http.request({
          hostname: '127.0.0.1', port: oldPort, path: '/api/shutdown', method: 'POST', timeout: 3000,
    ...
    for (let i = 0; i < 40; i++) {
      ...
      if (isPidDead(oldPid)) { console.log(`[boos] 旧实例 PID ${oldPid} 已退出`); oldPort = null; break; }
    }
  }
