...
// Sprint 42 robustness: append every reported error to a queryable JSONL
// file (~/.boos/errors.jsonl) so failures are greppable even when the
// server.log stream is lost. Line-buffered append, never throws.
const ERRORS_LOG = process.env.BOOS_ERRORS_LOG || path.join(os.homedir(), '.boos', 'errors.jsonl');

let _errorsFd = null;
function _appendError(level, module, op, message, extra) {
  try {
    if (_errorsFd === null) {
      fs.mkdirSync(path.dirname(ERRORS_LOG), { recursive: true });
      _errorsFd = fs.openSync(ERRORS_LOG, 'a');
    }
    const rec = JSON.stringify({
      ts: new Date().toISOString(), level, module, op,
      message: String(message).slice(0, 1000),
      extra: extra || null,
    });
    fs.writeSync(_errorsFd, rec + '\n');
  } catch { /* never let error-logging itself crash the caller */ }
}
...
function report(module, op, err, extra) {
  ...
  const msg = `${key} — ${fmtErr(err)}${suffix}`;
  console.error(`[err] ${msg}`);
  _appendError('err', module, op, fmtErr(err), extra);
}
...
function skip(module, op, reason, extra) {
  ...
  _appendError('warn', module, op, 'skipped: ' + reason, extra);
}
