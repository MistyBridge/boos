...
//     projectSlug: null,        // ~/.claude/projects/<slug>/ for Claude sessions.
//                              //   Discovered by sessionBinding Phase 2; null for
//                              //   non-Claude CLIs and until first Phase 2 scan.
//     manualStopped: false,    // true only when the user explicitly stopped
//                              //   it from boos; prevents auto-resume until
//                              //   they press Resume.
//     agentUid: null,          // Sprint 22: agent-bus persistent UID.
//                              //   Set when an agent session is launched via
//                              //   agent-bus; null for user-launched sessions.
//   }
...
function normalizeEntry(entry) {
  ...
  if (!('projectSlug' in out)) out.projectSlug = null;
  if (!('agentUid' in out)) out.agentUid = null;
  return out;
}
...
function buildEntry(opts) {
  const {
    ...
    status = 'running', cliSessionId = null, agentUid = null,
  } = opts;
  return {
    id: genId(),
    cliId,
    ...
    // Sprint 7: project slug for Claude sessions — discovered by
    // sessionBinding Phase 2 (project-dir fallback scan).
...
// Sprint 33: Find session by cliSessionId (Claude --resume UUID).
// This is the primary lookup when agent UID = cliSessionId.
async function findByCliSessionId(cliSessionId) {
  if (!cliSessionId) return null;
  const list = await loadAll();
  return list.find((s) => s.cliSessionId === cliSessionId && !s.deletedAt) || null;
}
...
// Sprint 7: persist the project slug (~/.claude/projects/<slug>/) discovered
// by sessionBinding Phase 2. Used to locate the JSONL file for PG sync.
async function setProjectSlug(id, projectSlug) {
  return update(id, { projectSlug: projectSlug || null });
}

// Sprint 9: Find the best matching BOOS session for an agent by name + workspace.
// Encapsulates session-lookup heuristics so callers (notifications.js) don't
// need to know the internal persisted-session schema.
// Returns the matched session record, or null.
//
// Post-restart edge case: the boot reconciliation in server.js marks all
// persisted sessions as 'exited' on startup. When sessions are auto-resumed
// by the frontend, the PTY is spawned but the persisted status may lag
// behind. This function checks both persisted status AND actual PTY liveness
// (via webTerminal when available) so wake_agent works immediately after
// a BOOS restart without requiring the status fix to land first.
/**
 * @deprecated Sprint 22: Use session.agentUid field for O(1) lookup.
 *   All agent-launched sessions now carry agentUid. This heuristic
 *   exists only for pre-migration sessions without the field.
 */
async function findByAgentName(agentName, workspace) {
  const path = require('node:path');
  const all = await loadAll();

  // ── Pass 0: persisted status === 'running' ──────────────────────
  const running = all.filter((s) => s.status === 'running' && s.cwd);

  function _match(candidates) {
    // Sub-pass A: cwd basename === agentName AND workspace matches.
    const exact = candidates.filter(
      (s) => path.basename(s.cwd) === agentName && s.workspace === workspace,
    );
    if (exact.length > 0) return exact[0];
    // Sub-pass B: cwd basename === agentName.
    const nameMatch = candidates.filter((s) => path.basename(s.cwd) === agentName);
    if (nameMatch.length > 0) return nameMatch[0];
    // Sub-pass C: fuzzy substring match (case-insensitive).
  ...
  const match = _match(running);
  if (match) return match;

  // ── Pass 1: persisted status !== 'running' but PTY is actually alive ──
  // Handles the post-restart gap where persisted status is still 'exited'
  // but the session was auto-resumed and the PTY is running.
  try {
    const wt = require('./webTerminal');
    const errReport = require("./errorReport");
    const liveIds = new Set(
      wt.list().filter((t) => !t.exitedAt).map((t) => t.id),
    );
    if (liveIds.size > 0) {
      const liveButStale = all.filter(
        (s) => s.cwd && liveIds.has(s.id) && s.status !== 'running',
      );
      const liveMatch = _match(liveButStale);
      if (liveMatch) {
        // Opportunistically fix the stale status for next lookup.
        try {  await markRunning(liveMatch.id);  } catch (e) { errReport.report("persistedSessions", "markRunning", e); }
        return liveMatch;
      }
    }
  } catch {
    // webTerminal not available (e.g. in test context) — no-op.
  }

  return null;
}

// Sprint 22: set agent-bus UID on an existing session record.
async function setAgentUid(sessionId, agentUid) {
  return withFileLock(FILE, async () => {
    const list = pruneExpiredDeleted(await readAllEntries());
    const entry = list.find((s) => s.id === sessionId);
    if (!entry) return false;
    entry.agentUid = agentUid;
    await saveAll(list);
    return true;
  });
}
