  ...
  async function scanSessionBindings() {
    ...
    try {
      ...
      for (const s of ordered) {
        ...
        let result = null;
        try {  result = await detectMod.detect(type, ptyPid, s.cwd, tree, detectOpts);  } catch (e) { errReport.report("sessionBinding", "detect", e); }
        const sid = result ? result.sessionId : null;

        if (sid && sid !== s.cliSessionId) {
          // Sprint 42 fix: the persisted cliSessionId IS the agent's
          // registered identity (Claude --resume UUID — registry card is
          // authoritative). The PID-file scan can surface the process-local
          // session UUID (~/.claude/sessions/<pid>.json, ephemeral), which
          // is NOT the identity — blindly overwriting flaps the record
          // between the two and crash-reconnect then injects the wrong
          // value into .mcp.json (observed: PM session bound to a2d49096,
          // team tasks to 82b97d58 unreachable). Rule: keep an existing
          // REGISTERED cliSessionId when the detected id has no agent card.
          const store = require('./agentBus/store');
          let existingRegistered = null;
          let detectedRegistered = null;
          try {
            existingRegistered = s.cliSessionId ? store.getAgent(s.cliSessionId) : null;
            detectedRegistered = sid ? store.getAgent(sid) : null;
          } catch { /* registry unavailable — fall through to plain update */ }
          if (existingRegistered && !detectedRegistered) {
            console.log(`[boos] binding kept · session ${s.id} · detected ${sid} is not a registered agent (keeping ${s.cliSessionId})`);
          } else {
            ...
            } catch (e) { errReport.report("sessionBinding", "setProjectSlug", e); }
            if (duplicateCodexCwd) {
              if (!claimedCodexIds.has(key)) claimedCodexIds.set(key, new Set());
          ...
          try {  await persistedSessions.setProjectSlug(s.id, result.projectSlug);  } catch (e) { errReport.report("sessionBinding", "setProjectSlug", e); }
        }
      }

      // Incremental sync to SQLite after each scan cycle.
      try {
      ...
      } catch (e) { errReport.report("sessionBinding", "log", e); }
