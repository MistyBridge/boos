...
// Read the first `maxLines` lines (up to maxBytes total). Returns the
// concatenated text (including newlines). Used by the project-dir fallback
// in detectClaude to skip past the metadata envelope line to find the cwd.
...
//
// FALLBACK (v1.0.1): Claude 2.x often does NOT write PID files when spawned
// through cmd.exe (like BOOS does). When the PID-based scan returns nothing,
// fall back to scanning ~/.claude/projects/<slug>/<uuid>.jsonl — those files
// ALWAYS exist and contain the correct cwd + session UUID. Extract the UUID
// from the filename of the most-recently-modified JSONL whose cwd matches.
async function detectClaude(descSet, cwd) {
  ...
  // Claude 2.x writes one JSONL per session under ~/.claude/projects/<slug>/.
  // The filename IS the session UUID. The first line's `cwd` field ties it to
  // the working directory. This fallback works even when the PID registry is
  // empty (e.g. Claude spawned through cmd.exe, or PID files cleaned up).
  const projectsDir = path.join(os.homedir(), '.claude', 'projects');
  let slugs;
  try {
    slugs = await fsp.readdir(projectsDir, { withFileTypes: true });
  } catch {
    return null;
  }
  ...
  for (const slug of slugs) {
    ...
    let files;
    try {
      files = await fsp.readdir(slugDir, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const f of files) {
      ...
      let stat;
      try {
        stat = await fsp.stat(path.join(slugDir, f.name));
      } catch {
        continue;
      }
      // Read first several lines to find the cwd — the very first line is
      // often a metadata envelope (type/mode/sessionId) with no cwd; cwd
      // appears on the following message envelope lines.
  ...
  // On Windows, path.resolve() can behave unexpectedly with non-ASCII paths
  // (e.g. Chinese characters) when running under MSYS2/bash. Normalize both
  // sides by resolving to absolute, lowercasing, and replacing all backslashes
  // with forward slashes before comparing.
  const norm = (p) => {
    try {
      return path.resolve(String(p || '')).toLowerCase().replace(/\\/g, '/');
    } catch { return String(p || '').toLowerCase().replace(/\\/g, '/'); }
  };
  ...
  async function scanSessionBindings() {
    ...
    try {
      ...
      for (const s of ordered) {
        ...
        try {
          result = await detect(type, ptyPid, s.cwd, tree, detectOpts);
        } catch {}
        ...
        if (sid && sid !== s.cliSessionId) {
          ...
          // We have a sessionId match but no slug stored yet (e.g. Phase 1 found
          // the sessionId earlier but couldn't know the slug). Fill it in now.
          try {
            await persistedSessions.setProjectSlug(s.id, result.projectSlug);
          } catch {}
        }
      }
      // Sprint 7: incremental sync to PostgreSQL after each scan cycle.
      try {
