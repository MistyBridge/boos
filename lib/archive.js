...
function listArchive(type, opts = {}) {
  ...
  for (const month of months) {
    ...
    for (const f of files.sort().reverse()) {
      ...
      } catch (e) { errReport.report("archive", "push", e); }
    }
  }
...
function pruneExpired() {
  ...
  for (const type of types) {
    ...
    for (const month of months) {
      ...
      } catch (e) { errReport.report("archive", "rmdirSync", e); }
    }
  }
...
function startPeriodicPrune() {
  ...
  // Run once at startup.
  try {  const r = pruneExpired(); if (r.removed > 0) console.log('[archive] startup prune: removed', r.removed, 'expired items');  } catch (e) { errReport.report("archive", "log", e); }
  // Then every 24 hours.
  _pruneTimer = setInterval(() => {
    try {  const r = pruneExpired(); if (r.removed > 0) console.log('[archive] periodic prune: removed', r.removed, 'expired items');  } catch (e) { errReport.report("archive", "log", e); }
  }, 24 * 60 * 60 * 1000).unref();
}
