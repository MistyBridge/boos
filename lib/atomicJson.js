  ...
  for (;;) {
    ...
    try {
      ...
      if (Date.now() - stat.mtimeMs > staleLockTimeoutMs) {
        ...
        try {  await fs.unlink(lockPath);  } catch (e) { errReport.report("atomicJson", "unlink", e); }
        continue; // Retry acquisition.
      }
    } catch (e) { errReport.report("atomicJson", "unlink", e); }
...
async function atomicWriteJson(filePath, data) {
  ...
  // Ensure the parent dir exists before creating the sibling tmp/lock
  // files — otherwise a fresh DATA_DIR (e.g. test BOOS_HOME) ENOENTs
  // on the lock write. fsSync is already imported above.
  ...
  try {
    ...
    try {  await fs.unlink(tmp);  } catch (e) { errReport.report("atomicJson", "unlink", e); }
    throw e;
  } finally {
