...
function _docker(args, timeoutMs = 30_000) {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      try {  child.kill();  } catch (e) { errReport.report("postgres", "kill", e); }
      finish(-1);
    }, timeoutMs);
...
async function _startContainer() {
  ...
  const pgData = require('node:path').join(require('./config').DATA_DIR, 'pgdata');
  require('node:fs').mkdirSync(pgData, { recursive: true });
...
async function _initPool() {
  ...
  // the DB connection is terminated (e.g. Docker stop, network blip).
  pool.on('error', (err) => {
    console.warn('[boos] postgres: pool error —', err.message);
  });
  // Verify connectivity.
  try {
...
async function _runDDL(pool) {
  ...
  // Sprint 33: identity index DDL.
  try {
    const { runDDL: runIdentityDDL } = require('./identityAdapter');
    const errReport = require("./errorReport");
    await runIdentityDDL(pool);
  } catch (e) {
    console.warn('[boos] postgres: identity DDL failed —', e.message);
    return false;
  }
...
async function stopContainer() {
  if (_pool) {
    try {  await _pool.end();  } catch (e) { errReport.report("postgres", "end", e); }
    _pool = null;
  }
  if (_available) {
