...
function _runDDL(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS conversation_turns (
      ...
      content_text   TEXT,
      raw_json       TEXT,
      PRIMARY KEY (cli_session_id, turn_index)
    );
    CREATE TABLE IF NOT EXISTS conversation_files (
      cli_session_id  TEXT PRIMARY KEY,
      boos_session_id TEXT,
      project_slug    TEXT,
      cwd             TEXT,
      jsonl_path      TEXT,
...
function getDb() {
  ...
  try {
    ...
    // callers degrade to their JSON fallback path. Warn ONCE, not per call
    // (boot floods the log otherwise).
    if (!_unavailableWarned) {
      _unavailableWarned = true;
      console.warn('[sqliteStore] unavailable:', e.message,
        '— 请用 boos / node --experimental-sqlite server.js 启动以启用 SQLite 身份索引');
    }
    return null;
  }
  return _db;
