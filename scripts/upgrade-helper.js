...
//   2. Helper writes ~/.boos/.upgrade.lock (so a stray boos:// wake on
//      boos.cmd doesn't try to start a new server while we're installing).
...
// ── Lockfile so bin/boos.js refuses to spawn a new server mid-upgrade.
...
<body>
<div class="card">
  <h1>Upgrading boos</h1>
  <p class="subtitle">target: <span class="mono">@MistyBridge/boos@<span id="target"></span></span></p>
...
(async () => {
  ...
  pushLine('info', `Upgrading boos to ${target}`);
  ...
  setPhase('installing');
  pushLine('info', `Running: npm i -g @MistyBridge/boos@${target}${installPrefix ? ` --prefix=${installPrefix}` : ''}`);
  ...
  const arg = `@MistyBridge/boos@${target}`;
  const npmArgs = ['i', '-g'];
  if (installPrefix) {
  ...
  const npmEnv = { ...process.env, BOOS_NO_AUTOLAUNCH: '1' };
  ...
  const boosCmd = installPrefix
    ? (isWin ? path.join(installPrefix, 'boos.cmd') : path.join(installPrefix, 'bin', 'boos'))
    : (isWin ? 'boos.cmd' : 'boos');
  const childEnv = { ...process.env };
  delete childEnv.BOOS_NO_BROWSER;
  // Hint the new boos that it was spawned by the updater so it can
  // skip auto-opening a browser window — the user is already looking
  // at the updater UI which will redirect to the new server.
  childEnv.BOOS_FROM_UPGRADE = '1';
  let respawnExe, respawnArgs;
  if (isWin) {
    respawnExe = process.env.ComSpec || 'cmd.exe';
    respawnArgs = ['/d', '/s', '/c', boosCmd];
  } else {
    respawnExe = boosCmd;
    respawnArgs = [];
  }
  try {
    ...
    child.unref();
    pushLine('info', `Spawned ${boosCmd} (via ${path.basename(respawnExe)}).`);
  } catch (e) {
