# boos — Codex Session Manager

A small Node/Express + Preact web tool that runs every Codex/Codex/
...
When you're running 8–10 concurrent `Codex` sessions across ad-hoc
...
- `clis` — array of CLI definitions. Built-ins for `Codex`, `codex`, `copilot`; users can add `other` CLIs with custom `command`, `args`, `resumeLatestArgs`, `resumePickerArgs`, `shell` (direct/pwsh/cmd).
...
- Codex: exact `--resume <id>`, latest `--continue`, picker `--resume`
...
The UI deliberately copies **Codex.ai's** calm light aesthetic — warm
