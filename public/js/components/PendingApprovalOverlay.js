  ...
  return html`
    <div class="offline-overlay" role="dialog" aria-modal="true" aria-live="polite">
      <div class="offline-card">
        ...
        ${staleToken ? html`
          <h1 class="offline-title">${T.pendingApproval.linkExpired}</h1>
          <p class="offline-copy">
            ${T.pendingApproval.linkExpiredMsg}
          </p>
          <p class="offline-copy" style="margin-top:6px;font-size:12px;color:var(--ink-muted)">
            ${T.pendingApproval.askNewLink}
          </p>
        ` : rejected ? html`
          <h1 class="offline-title">${T.pendingApproval.accessDeclined}</h1>
          <p class="offline-copy">
            ${T.pendingApproval.accessDeclinedMsg}
          </p>
        ` : html`
          <h1 class="offline-title">${T.pendingApproval.waitingApproval}</h1>
          <p class="offline-copy">
            ${T.pendingApproval.waitingMsg}
          </p>
          ${getDeviceCode() ? html`
            <div class="offline-code-block">
              <div class="offline-code-label">${T.pendingApproval.yourCode}</div>
              <div class="offline-code">${getDeviceCode()}</div>
              <div class="offline-code-hint">${T.pendingApproval.codeHint}</div>
            </div>
          ` : null}
          <p class="offline-copy" style="margin-top:6px;font-size:12px;color:var(--ink-muted)">
            ${T.pendingApproval.autoUnlock}
          </p>
        `}
      </div>
    </div>`;
