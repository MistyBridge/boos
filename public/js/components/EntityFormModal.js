...
export function EntityFormModal({
  title, fields, initial = {}, submitLabel = '保存',
  readOnlyKeys = [],
  onSubmit, onClose, onTest, testLabel = '测试',
  danger,
}) {
  ...
  const footer = html`
    ...
    <button type="button" class="action small subtle" onClick=${onClose}>取消</button>
        ...
        ${testResult ? html`
          <div class=${`entity-test-result ${testResult.ok ? 'is-ok' : 'is-fail'}`}>
            <div class="entity-test-summary">
              ${testResult.ok ? '✓' : '✗'} ${testResult.ok ? '正常' : '失败'}
              ${typeof testResult.exitCode === 'number' ? html` · 退出码 ${testResult.exitCode}` : null}
              ${typeof testResult.durationMs === 'number' ? html` · ${testResult.durationMs}ms` : null}
              ${testResult.timedOut ? html` · 超时` : null}
              ${testResult.matchedType === true ? html` · 类型匹配 ${testResult.expectedType}` : null}
              ${testResult.matchedType === false ? html` · 类型不匹配（预期 ${testResult.expectedType}）` : null}
            </div>
