...
export function PickerPanel({
  ...
  multi = false,
  showSearch = true,
  emptyHint = T.picker.nothing,
  createLabel = T.picker.newItem,
  createFields = [],
  onSelect,
  onToggle,
  onCreate,
    ...
    <div class="picker">
      ...
      ${showSearch ? html`
        <div class="picker-search">
          ...
          <input ref=${searchRef} class="picker-search-input"
                 placeholder=${T.picker.search} value=${q}
      ...
      <div class="picker-list">
        ${filtered.length === 0 ? html`
          <div class="picker-empty">${q ? T.picker.noMatches : emptyHint}</div>
        ` : filtered.map((it) => {
            ...
            <form class="picker-create-form" onSubmit=${submitCreate}>
              ...
              <div class="picker-create-actions">
                ...
                <button type="submit" class="action small primary" disabled=${saving}>
                  ${saving ? T.picker.creating : T.picker.create}
                </button>
              </div>
            </form>`}
        </div>` : null}
