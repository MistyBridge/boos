...
export function ReposEditor({ onChange }) {
  ...
  return html`
    <table class="data repos-table">
      <thead>
        <tr>
          <th>${T.entityForm.name}</th>
          <th>${T.entityForm.url}</th>
          <th class="num">${T.entityForm.default}</th>
  ...
  return html`
    <tr>
      ...
      <td><div class="row-actions">
        <button class="action tiny danger" onClick=${() => remove(idx)}>${T.entityForm.remove}</button>
      </div></td>
    </tr>`;
}
