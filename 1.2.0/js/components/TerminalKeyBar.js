...
const CTRL_COMBOS = [
  { label: '^C', data: '\x03', hint: T.keyBar.interrupt },
  { label: '^D', data: '\x04', hint: T.keyBar.eof },
  { label: '^Z', data: '\x1a', hint: T.keyBar.suspend },
  { label: '^R', data: '\x12', hint: T.keyBar.revSearch },
  { label: '^L', data: '\x0c', hint: T.keyBar.clear },
  { label: '^A', data: '\x01', hint: T.keyBar.lineStart },
  { label: '^E', data: '\x05', hint: T.keyBar.lineEnd },
  { label: '^U', data: '\x15', hint: T.keyBar.killLine },
  { label: '^K', data: '\x0b', hint: T.keyBar.killToEnd },
  { label: '^W', data: '\x17', hint: T.keyBar.killWord },
];
    ...
    <div class="term-keybar" style=${`bottom:${kbOffset}px`}>
      ...
      <div class="term-keybar-row">
        <button class=${`tkb-key${ctrlOpen ? ' is-active' : ''}`}
                ...${keyProps(() => setCtrlOpen((v) => !v))}>${T.keyBar.ctrl}</button>
        <button class="tkb-key" ...${sendKey('\x1b')}>${T.keyBar.esc}</button>
        <button class="tkb-key" ...${sendKey('\t')}>${T.keyBar.tab}</button>
        <button class="tkb-key tkb-wide" ...${sendKey('\x1b[Z')}>${T.keyBar.sTab}</button>
        <button class="tkb-key tkb-arrow" ...${sendKey(newlineData)} aria-label=${T.keyBar.newline}><span class="tkb-glyph">↵</span></button>
        <button class="tkb-key tkb-arrow" ...${sendKey('\x1b[A')} aria-label=${T.keyBar.up}><${IconChevronUp} /></button>
        <button class="tkb-key tkb-arrow" ...${sendKey('\x1b[B')} aria-label=${T.keyBar.down}><${IconChevronDown} /></button>
        <button class="tkb-key tkb-arrow" ...${sendKey('\x1b[D')} aria-label=${T.keyBar.left}><${IconChevronLeft} /></button>
        <button class="tkb-key tkb-arrow" ...${sendKey('\x1b[C')} aria-label=${T.keyBar.right}><${IconChevronRight} /></button>
      </div>
    </div>`;
}
