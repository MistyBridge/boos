...
export function DirectoryPicker({ initialPath, onPick }) {
  ...
  if (!data) {
    return html`<div class="filex"><div class="filex-loading">${T.dirPicker.loading}</div></div>`;
  }
    ...
    <div class="filex">
      <div class="filex-toolbar">
        <div class="filex-navbtns">
          <button class="filex-navbtn" title=${T.dirPicker.back} disabled=${!canBack} onClick=${goBack}>
            <${IconChevronLeft} />
          </button>
          <button class="filex-navbtn" title=${T.dirPicker.forward} disabled=${!canForward} onClick=${goForward}>
            <${IconChevronRight} />
          </button>
          <button class="filex-navbtn" title=${T.dirPicker.up} disabled=${!data.parent} onClick=${goUp}>
            <${IconChevronUp} />
          </button>
        </div>
          ...
          <div class="filex-breadcrumb"
            ...
            ${segments.map((seg, i) => html`
              ...
              ${i < segments.length - 1
                ...
                : null}
            `)}
            <button class="filex-address-edit-btn" title=${T.dirPicker.editPath}
      ...
      <div class="filex-body">
        <aside class="filex-side">
          <div class="filex-side-label">${T.dirPicker.quickAccess}</div>
        ...
        <div class="filex-main">
          ${!data.exists ? html`
            <div class="filex-empty">${T.dirPicker.dirNotFound}</div>
          ` : html`
            <div class="filex-list">
              ${data.entries.length === 0 ? html`
                <div class="filex-empty">${T.dirPicker.folderEmpty}</div>
