...
import { api, loadConfig, refreshAll, loadSessions, loadDeletedSessions, loadFolders, loadWorkspaces, loadTasks, pollHealth, pendingDevice, fetchUnreadDecisionCount } from './api.js';
...
(async () => {
  ...
  setInterval(async () => {
    ...
    try {
      await Promise.all([loadSessions(), loadDeletedSessions(), loadFolders(), loadWorkspaces(), loadTasks(), fetchUnreadDecisionCount()]);
